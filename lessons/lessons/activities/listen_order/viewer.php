...
.lo-audio-icon{
    ...
    border-radius:50%;
    background:#7F77DD;
    box-shadow:0 6px 20px rgba(127,119,221,.28);
    display:flex;
    align-items:center;
    justify-content:center;
    flex-shrink:0;
...
.lo-card{
    ...
    background:#ffffff;
    box-shadow:0 4px 14px rgba(127,119,221,.10);
    cursor:pointer;
    position:relative;
    display:flex;
    flex-direction:column;
...
.lo-card:hover{
    ...
    box-shadow:0 6px 20px rgba(127,119,221,.16);
}
...
.lo-card.selected{
    border-color:#F97316;
    background:#FFF8F4;
    box-shadow:0 0 0 3px rgba(249,115,22,.18),0 4px 14px rgba(127,119,221,.10);
...
body.presentation-mode .lo-board{
    overflow:visible!important;
}
body.embedded-mode .lo-actions,
body.fullscreen-embedded .lo-actions,
body.presentation-mode .lo-actions{
...
<div class="lo-shell">
    <div class="lo-app">
        ...
        <div class="lo-hero">
            ...
            <p class="lo-subtitle">Listen and tap images to put them in the correct order.</p>
        </div>
        ...
        <div class="lo-board" id="lo-board">
            ...
            <div id="lo-audio-player" class="lo-audio-player">
                ...
                <div class="lo-audio-info">
                    ...
                    <div class="lo-audio-track">
                        <div class="lo-audio-fill" id="lo-audio-fill"></div>
                    </div>
            ...
            <div id="lo-video-player" class="lo-video-player" style="display:none">
                <div class="lo-video-overlay">
                    <div class="lo-video-play-btn">▶</div>
                </div>
            </div>
        ...
        <div id="lo-completed" class="lo-completed">
            ...
            <div class="lo-done-track">
                <div class="lo-done-fill" id="lo-done-fill"></div>
            </div>
...
function _loLoadVoices() {
    if ('speechSynthesis' in window) { _loVoices = window.speechSynthesis.getVoices() || []; }
}
...
function getPreferredVoice() {
    ...
    for (var p = 0; p < preferred.length; p++) {
        ...
        for (var v = 0; v < enVoices.length; v++) {
            ...
            if (label.indexOf(hint) !== -1) return enVoices[v];
        }
    }
...
var feedbackEl    = document.getElementById('lo-feedback');
var statusEl      = document.getElementById('lo-status');
var kickerCountEl = document.getElementById('lo-kicker-count');
var hintEl        = document.getElementById('lo-hint');
var scoresEl      = document.getElementById('lo-scores');
var scCorrectEl   = document.getElementById('lo-score-correct');
var scWrongEl     = document.getElementById('lo-score-wrong');
var scPctEl       = document.getElementById('lo-score-pct');
var completedEl   = document.getElementById('lo-completed');
var completedTitleEl = document.getElementById('lo-completed-title');
var completedTextEl  = document.getElementById('lo-completed-text');
var scoreTextEl   = document.getElementById('lo-score-text');
...
var audioFillEl   = document.getElementById('lo-audio-fill');
var audioNameEl   = document.getElementById('lo-audio-name');
...
function setListenBtnState(state) {
    ...
    if (state === 'speaking') {
        ...
        if (audioFillEl) audioFillEl.style.width = '50%';
...
function playAudio() {
    if (finished) return;
    if (!currentSentence || String(currentSentence).trim() === '') return;
    ...
    if (speechSynthesis.paused || isPaused) {
        ...
        setTimeout(function () {
            if (!speechSynthesis.speaking && speechOffset < speechSourceText.length) startSpeechFromOffset();
        }, 80);
        return;
    }
    if (speechSynthesis.speaking && !speechSynthesis.paused) {
        speechSynthesis.pause(); isSpeaking = true; isPaused = true;
        setListenBtnState('paused'); return;
    }
...
function updateHint() {
    if (!hintEl) return;
    if (selectedIdx === null) {
        hintEl.innerHTML = '<span class="lo-hint-neutral">Tap an image to select it, then tap another to swap</span>';
    } else {
        hintEl.innerHTML = '<span class="lo-hint-selected">Position ' + (selectedIdx + 1) + ' selected — tap another to swap</span>';
    }
}
...
function renderGrid(states) {
    ...
    userOrder.forEach(function (src, i) {
        ...
        if (states) {
            if (states[i] === 'correct') card.classList.add('correct');
            else if (states[i] === 'wrong') card.classList.add('wrong');
...
function onCardTap(i) {
    ...
    if (selectedIdx === null) {
        ...
        var tmp = userOrder[selectedIdx];
        userOrder[selectedIdx] = userOrder[i];
        userOrder[i] = tmp;
        selectedIdx = null;
...
function checkAnswer() {
    ...
    for (var i = 0; i < correct.length; i++) {
        if (userOrder[i] === correct[i]) { states.push('correct'); correctItems++; }
        else { states.push('wrong'); }
    }
    var wrongItems = correct.length - correctItems;
    var pct = correct.length > 0 ? Math.round((correctItems / correct.length) * 100) : 0;
    var allCorrect = correctItems === correct.length;
    ...
    var attempts = (attemptsByBlock[index] || 0) + 1;
    attemptsByBlock[index] = attempts;
    ...
    if (allCorrect) {
        ...
        feedbackEl.textContent = '✘ Wrong — see the correct order below';
        ...
        feedbackEl.textContent = '✘ Not quite — try again (attempt 1/2)';
        feedbackEl.className = 'bad';
        playSound(loseSound);
        renderGrid(states);
    }
...
function updateStatus() {
    var text = (index + 1) + ' / ' + totalCount;
    if (statusEl)      statusEl.textContent      = text;
    if (kickerCountEl) kickerCountEl.textContent = text;
}
...
function loadBlock() {
    ...
    speechSourceText = ''; speechSegmentStart = 0;
...
async function showCompleted() {
    finished = true; blockFinished = true;
    ...
    if (LO_ACTIVITY_ID && LO_RETURN_TO) {
        ...
        var saveUrl = LO_RETURN_TO + joiner +
...
function nextBlock() {
    if (blockFinished || checkedBlocks[index]) {
        if (index >= blocks.length - 1) { showCompleted(); return; }
        index += 1;
        loadBlock();
    } else {
        feedbackEl.textContent = 'Check your answer first.';
        feedbackEl.className = 'bad';
    }
}
...
function restartActivity() {
    index = 0; correctCount = 0; totalCount = blocks.length;
    attemptsByBlock = {}; checkedBlocks = {};
