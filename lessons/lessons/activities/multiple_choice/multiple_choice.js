document.addEventListener('DOMContentLoaded', function () {
  ...
  let activeAudioUrl = ''; // pre-downloaded URL if available

  function playSound(audio) {
    try { audio.pause(); audio.currentTime = 0; audio.play(); } catch (e) {}
  }

  if (completedTitleEl) { completedTitleEl.textContent = activityTitle; }
  if (completedTextEl) { completedTextEl.textContent = "You've completed " + activityTitle + '. Great job practicing.'; }

  function playCompletedSound() { playSound(completedSound); }
  ...
  function navigateToReturn(targetUrl) {
    if (!targetUrl) { return; }
    try { if (window.top && window.top !== window.self) { window.top.location.href = targetUrl; return; } } catch (e) {}
  ...
  function computeScore() {
    ...
    let correct = 0, wrong = 0, revealedCount = 0;
    questionScores.forEach(function (value) {
      if (value === 1) correct += 1;
      else if (value === 0) wrong += 1;
      else if (value === -1) revealedCount += 1;
    });
    const scorable = correct + wrong;
    const percent = scorable > 0 ? Math.round((correct / scorable) * 100) : 0;
    return { correct: correct, total: total, wrong: wrong, errors: wrong, revealed: revealedCount, percent: percent };
  }

  function updateScoreCards(show) {
    if (typeof show === 'boolean') { scoreVisible = show; }
  ...
  function safeOptions(item) { return item && Array.isArray(item.options) ? item.options : []; }
  ...
  function renderOptions() {
    ...
    safeOptions(item).forEach(function (optionText, optIndex) {
      ...
      if (selected === optIndex) { button.classList.add('selected'); }
      if ((revealed || score === -1) && optIndex === correct) { button.classList.add('correct'); }
      if (score === 1 && selected === optIndex) { button.classList.add('correct'); }
      if (score === 0) {
        if (selected === optIndex) { button.classList.add('wrong'); }
        if (optIndex === correct) { button.classList.add('correct'); }
      }
      ...
      button.addEventListener('click', function () {
        if (finished || answeredCurrent) { return; }
  ...
  function validateSelection(optIndex) {
    ...
    selected = optIndex; revealed = false; answeredCurrent = true;
    questionScores[index] = isCorrectPick ? 1 : 0;
    renderOptions(); updateScoreCards(true);
    if (feedbackEl) {
      if (isCorrectPick) { feedbackEl.textContent = 'Correct! Great job.'; feedbackEl.className = 'mc-feedback good'; }
      else { feedbackEl.textContent = 'Incorrect. Correct option highlighted.'; feedbackEl.className = 'mc-feedback bad'; }
    }
    if (showBtn) { showBtn.disabled = true; }
  ...
  function loadQuestion() {
    ...
    selected = null; revealed = false; answeredCurrent = false; finished = false;
    activeListenText = cleanQuestion;
    activeVoiceId = String(item.voice_id || 'josh');
    // Use pre-downloaded audio URL if available (starts with http)
    const storedAudio = String(item.audio || '').trim();
    activeAudioUrl = (storedAudio.indexOf('http') === 0) ? storedAudio : '';

    if (completedEl) { completedEl.classList.remove('active'); }
    if (cardEl) { cardEl.style.display = 'block'; }
    if (controlsEl) { controlsEl.style.display = 'flex'; }
    if (feedbackEl) { feedbackEl.textContent = ''; feedbackEl.className = 'mc-feedback'; }
    updateProgress();
    if (questionEl) { questionEl.textContent = cleanQuestion || 'Choose the correct answer.'; }
    if (listenBtn) { listenBtn.disabled = !isListen || cleanQuestion === ''; }
    if (item.image) { imageEl.src = item.image; imageBoxEl.classList.remove('is-empty'); }
    else { imageEl.removeAttribute('src'); imageBoxEl.classList.add('is-empty'); }
    renderOptions();
    if (showBtn) { showBtn.disabled = false; }
    if (nextBtn) { nextBtn.textContent = index < questions.length - 1 ? 'Next →' : 'Finish'; nextBtn.disabled = true; }
  }

  function showAnswer() {
    if (finished || answeredCurrent) { return; }
    selected = null; revealed = true; answeredCurrent = true; questionScores[index] = -1;
    renderOptions(); updateScoreCards(true);
    if (feedbackEl) { feedbackEl.textContent = 'Answer revealed — this question does not affect score.'; feedbackEl.className = 'mc-feedback'; }
    if (showBtn) { showBtn.disabled = true; }
    if (nextBtn) { nextBtn.disabled = false; }
  }
  ...
  async function showCompleted() {
    ...
    if (scoreTextEl) { scoreTextEl.textContent = result.correct + ' correct · ' + result.wrong + ' wrong · ' + result.percent + '%'; }
    playCompletedSound();
  ...
  function nextQuestion() {
    if (finished) { return; }
    if (!answeredCurrent) {
      if (feedbackEl) { feedbackEl.textContent = 'Select an option or tap Show Answer first.'; feedbackEl.className = 'mc-feedback bad'; }
      return;
    }
    if (index < questions.length - 1) { index += 1; loadQuestion(); return; }
    showCompleted();
  }

  function restartActivity() {
    index = 0; selected = null; revealed = false; answeredCurrent = false;
    finished = false; scoreVisible = false;
    questionScores = questions.map(function () { return null; });
    updateScoreCards(false); loadQuestion();
  }
  ...
  if (listenBtn) {
    listenBtn.addEventListener('click', function () {
      if (!activeListenText) { return; }
      // If a pre-downloaded audio URL exists, play it directly — no ElevenLabs call needed
      if (activeAudioUrl) {
        playAudioUrl(activeAudioUrl);
      } else {
  ...
  function stopSpeech() {
    ...
    if (currentAudioElement) { currentAudioElement.pause(); currentAudioElement.currentTime = 0; currentAudioElement = null; }
  }

  // Play a pre-downloaded audio URL directly (no ElevenLabs call)
  ...
  function speakText(text, voiceId) {
    ...
    fetch('tts.php', { method: 'POST', body: formData, signal: signal })
  ...
  function speakWhenReady(textOrUrl, voiceId) {
    if (!textOrUrl) { return; }
    if (userInteracted) {
      if (activeAudioUrl) { playAudioUrl(activeAudioUrl); }
      else { speakText(textOrUrl, voiceId); }
    } else {
      pendingSpeech = { text: textOrUrl, voiceId: voiceId };
    }
  }
  ...
  document.addEventListener('click', function onFirstInteraction() {
    ...
    if (pendingSpeech && typeof pendingSpeech === 'object') {
      if (activeAudioUrl) { playAudioUrl(activeAudioUrl); }
      else { speakText(pendingSpeech.text, pendingSpeech.voiceId); }
      pendingSpeech = '';
    }
