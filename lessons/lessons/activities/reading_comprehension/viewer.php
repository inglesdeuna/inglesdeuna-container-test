...
<style>
  ...
  .rc-correct { min-width: 112px; border: 1.5px solid #DCD7FF; border-radius: 12px; background: #fff; color: #5B51C8; padding: 10px 8px; font-weight: 900; cursor: pointer; }
  .rc-correct.selected { border-color: #1D9E75; background: #E1F5EE; color: #085041; }
  .rc-validation { clear: both; margin: 8px 0 0; color: #B42318; font-weight: 900; font-size: 13px; }
...
(function () {
  ...
  function normalizeOption(x, qid, oi) { const raw = x && typeof x === 'object' ? x : { text: x }; return Object.assign({}, raw, { id: String(raw.id || raw.option_id || (qid + '_option_' + (oi + 1))), text: String(raw.text ?? raw.label ?? raw.value ?? '') }); }
  function legacyCorrectId(x, options) { const raw = x && typeof x === 'object' ? x : {}; const candidates = [raw.correct_option_id, raw.correctOptionId, raw.correctAnswer, raw.correct_answer, raw.answer]; for (const candidate of candidates) { if (candidate !== undefined && candidate !== null && String(candidate).trim() !== '') { const value = typeof candidate === 'object' ? (candidate.id ?? candidate.value ?? candidate.text) : candidate; const byId = options.find(o => o.id === String(value)); if (byId) return byId.id; const byText = options.find(o => o.text === String(value)); if (byText) return byText.id; const n = Number(value); if (Number.isInteger(n) && options[n]) return options[n].id; } } const legacyIndex = raw.correctIndex ?? raw.correct; const n = Number(legacyIndex); return Number.isInteger(n) && options[n] ? options[n].id : null; }
  function normalizeQuestion(x) { x = x || {}; const id = String(x.id || uid('q')); let source = Array.isArray(x.options) ? x.options : [x.option_a, x.option_b, x.option_c, x.option_d].filter(v => v !== undefined); if (!source.length) source = ['', '', '', '']; const options = source.map((op, oi) => normalizeOption(op, id, oi)); return Object.assign({}, x, { id, stem: String(x.stem ?? x.question ?? ''), options, correct_option_id: legacyCorrectId(x, options), feedback: String(x.feedback || '') }); }
  function normalizeText(x) { x = x || {}; const body = String(x.body || ''); const wc = Number(x.wordCount); return Object.assign({}, x, { id: String(x.id || uid('t')), mode: String(x.mode || 'vocab').toLowerCase() === 'comp' ? 'comp' : 'vocab', title: String(x.title || window.RC_SAVED_TITLE || 'Reading Comprehension'), genre: String(x.genre || 'Informative text'), wordCount: Number.isFinite(wc) && wc > 0 ? wc : wordsCount(body), body, words: (Array.isArray(x.words) ? x.words : []).map(normalizeWord), questions: (Array.isArray(x.questions) ? x.questions : []).map(normalizeQuestion) }); }
  function normalizeDataset(raw) { if (raw && Array.isArray(raw.texts) && raw.texts.length) return Object.assign({}, raw, { title: String(raw.title || window.RC_SAVED_TITLE || 'Reading Comprehension'), texts: raw.texts.map(normalizeText) }); return { title: String(window.RC_SAVED_TITLE || 'Reading Comprehension'), texts: [normalizeText(raw || {})] }; }
  ...
  function validationFor(q, qi) { if (!String(q.stem || '').trim()) return 'Please enter Question ' + (qi + 1) + '.'; if (!q.options.length || q.options.some(o => !String(o.text || '').trim())) return 'Please complete every answer option for Question ' + (qi + 1) + '.'; if (!q.correct_option_id || !q.options.some(o => o.id === q.correct_option_id)) return 'Please select the correct answer for Question ' + (qi + 1) + '.'; return ''; }
  function validateBeforeSave() { if (text().mode !== 'comp') return ''; for (let i = 0; i < text().questions.length; i++) { const error = validationFor(text().questions[i], i); if (error) return error; } return ''; }
  ...
  root.addEventListener('input', function(e) {
    ...
    if (el.dataset.question !== undefined) {
      ...
      if (!q) return;
      const prop = el.dataset.prop;
      if (prop === 'option') q.options[Number(el.dataset.option)].text = el.value;
  ...
  root.addEventListener('click', async function(e) {
    ...
    if (action === 'correct') {
      ...
      if (q && q.options[Number(btn.dataset.option)]) { q.correct_option_id = q.options[Number(btn.dataset.option)].id; render(); }
    }
    if (action === 'answer') {
      if (checked) return;
    ...
    if (action === 'save') {
      ...
      if (validation) { status = validation; render(); return; }
      if (saving) return;
      ...
      try {
        const resp = await fetch('save.php', {
          ...
          body: JSON.stringify(Object.assign({}, state, { id: window.RC_ACTIVITY_ID, title: text().title, texts: state.texts }))
        });
