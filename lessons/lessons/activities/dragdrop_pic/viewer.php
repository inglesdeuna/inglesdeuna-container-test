...
function renderBlock(){blockFinished=false;blockCorrect=0;blockMistakes=0;clearSelection();setFeedback('','');nextBtn.style.display='none';progress.textContent=`Block ${blockIndex+1} of ${BLOCKS.length}`;blockTitle.textContent=current().title||`Block ${blockIndex+1}`;blockInstructions.textContent=current().instructions||'';canvas.innerHTML='';const bg=document.createElement('img');bg.className='ddpv-bg';bg.src=current().background_image;bg.alt='Activity scene';canvas.appendChild(bg);items().forEach(i=>{const z=document.createElement('div');z.className='ddpv-zone';z.dataset.id=i.id;Object.assign(z.style,{left:i.x+'%',top:i.y+'%',width:i.w+'%',height:i.h+'%',zIndex:String(10+i.layer)});z.addEventListener('pointerup',e=>{if(finished||blockFinished||dragState||!selectedChip||z.classList.contains('filled'))return;e.preventDefault();attemptDrop(z,selectedChip)});canvas.appendChild(z)});buildBank()}
