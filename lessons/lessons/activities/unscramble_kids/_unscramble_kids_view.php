<?php
$uskQuizMode = !empty($uskQuizMode);
$uskTtsUrl = isset($uskTtsUrl) && trim((string)$uskTtsUrl) !== '' ? (string)$uskTtsUrl : 'tts.php';
?>
...
const USK_TTS_URL     = <?php echo json_encode($uskTtsUrl, JSON_UNESCAPED_SLASHES); ?>;
const USK_QUIZ_MODE   = <?php echo $uskQuizMode ? 'true' : 'false'; ?>;
...
function uskRender(){
    ...
    /* image overrides emoji */
    const imgBox = document.getElementById('uskImgBox');
    imgBox.innerHTML = '';
    if (w.image) {
...
function uskAutoCheck(){
    if (USK_QUIZ_MODE || uskBlocked) return;
...
function uskNext(){
    if (USK_QUIZ_MODE) {
        const answer = uskPlaced.map(bi => bi === null ? '' : uskOrder[bi].l).join('');
        if (typeof window.USK_QUIZ_SUBMIT === 'function') window.USK_QUIZ_SUBMIT(answer, false);
        return;
    }
...
function uskSpeak(){
    ...
    fetch(USK_TTS_URL, { method:'POST', body:fd, credentials:'same-origin' })
        ...
        .then(blob => {
            ...
            uskCurrentAudio.playbackRate = 0.8;
