...
function usk_ed_norm($raw): array {
    ...
    foreach (($d['words'] ?? []) as $it) {
        ...
        if ($w === '') continue;
        $words[] = ['id'=>trim((string)($it['id'] ?? uniqid('usk_'))),'word'=>$w,'image'=>trim((string)($it['image'] ?? '')),'hint'=>trim((string)($it['hint'] ?? '')),'audio'=>trim((string)($it['audio'] ?? '')),'voice_id'=>trim((string)($it['voice_id'] ?? ''))];
    }
...
if ($_SERVER['REQUEST_METHOD']==='POST') {
    ...
    $img_existing=is_array($_POST['item_image_existing'] ?? null) ? $_POST['item_image_existing'] : [];
    $img_uploads=isset($_FILES['item_image_upload']) ? $_FILES['item_image_upload'] : ['tmp_name'=>[],'error'=>[]];
    ...
    foreach($wt as $i => $rw) {
        ...
        if($w==='') continue;
        $imgUrl=trim((string)($img_existing[$i] ?? ''));
        $tmpName=$img_uploads['tmp_name'][$i] ?? '';
        $imgErr=$img_uploads['error'][$i] ?? UPLOAD_ERR_NO_FILE;
        if($tmpName!=='' && $imgErr===UPLOAD_ERR_OK){$uploaded=upload_to_cloudinary($tmpName);if($uploaded)$imgUrl=$uploaded;}
        $san[]=['id'=>trim((string)($ids[$i] ?? uniqid('usk_'))) ?: uniqid('usk_'),'word'=>$w,'image'=>$imgUrl,'hint'=>trim((string)($hi[$i] ?? '')),'audio'=>trim((string)($au[$i] ?? '')),'voice_id'=>trim((string)($vd[$i] ?? ''))];
    }
...
<div class="usk-row2"><div><label>Word</label><input type="text" name="word[]" value="<?= htmlspecialchars((string)($item['word'] ?? ''),ENT_QUOTES,'UTF-8') ?>" placeholder="CAT" style="text-transform:uppercase;font-family:'Fredoka';font-size:18px;font-weight:600" required></div><div><label>Image <span style="font-weight:400;color:#6b7280">(optional)</span></label><input type="hidden" name="item_image_existing[]" class="usk-img-existing" value="<?= htmlspecialchars((string)($item['image'] ?? ''),ENT_QUOTES,'UTF-8') ?>"><input type="file" name="item_image_upload[]" accept="image/*" class="usk-img-file" onchange="uskPrevImg(this)"><div class="usk-img-thumb"<?= empty($item['image']) ? ' style="display:none"' : '' ?>><?php if(!empty($item['image'])): ?><img src="<?= htmlspecialchars((string)$item['image'],ENT_QUOTES,'UTF-8') ?>" alt=""><?php endif; ?></div></div></div>
