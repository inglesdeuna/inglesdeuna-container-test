...
function os_normalize(mixed $rawData): array
{
    ...
    foreach ((array) ($d['sentences'] ?? []) as $s) {
        ...
        // Default to 'text' display if not provided
    ...
    return json_encode([
        ...
        'sentences'    => array_map(function($s) {
            return [
                ...
                // Always include display; default to 'text' if missing
                'display' => isset($s['display']) ? $s['display'] : 'text',
            ];
...
.os-back-btn:hover{filter:brightness(0.97);transform:translateY(-1px);} 
...
<form method="post" enctype="multipart/form-data" class="os-form" id="osSentencesForm">
    ...
    <div class="toolbar-row">
        <button type="button" class="os-back-btn" onclick="history.back()">← Back</button>
