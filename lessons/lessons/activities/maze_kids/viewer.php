...
function mzkBuildMaze(){const layout=generateMazeLayout(MZK_PATH,MZK_BRANCHES,MZK_LAYOUT_POSITIONS);const NS='http://www.w3.org/2000/svg';const svg=document.createElementNS(NS,'svg');svg.setAttribute('viewBox','0 0 '+layout.width+' '+layout.height);svg.setAttribute('width',layout.width);svg.setAttribute('height',layout.height);function addPath(d,stroke,w,dash,op){const p=document.createElementNS(NS,'path');p.setAttribute('d',d);p.setAttribute('fill','none');p.setAttribute('stroke',stroke);p.setAttribute('stroke-width',w);p.setAttribute('stroke-linecap','round');p.setAttribute('stroke-linejoin','round');if(dash)p.setAttribute('stroke-dasharray',dash);if(op)p.setAttribute('opacity',op);svg.appendChild(p);}addPath(layout.wallPathD,'var(--wall)',32,'',0.35);addPath(layout.corridorPathD,'var(--mz-purple)',3,'2 10',0.55);function addBlockCap(pt){const cap=document.createElementNS(NS,'g');cap.setAttribute('class','mzk-wall-cap');cap.setAttribute('transform','translate('+pt.x+','+pt.y+')');const c=document.createElementNS(NS,'circle');c.setAttribute('r',9);c.setAttribute('fill','#fff');c.setAttribute('stroke','var(--mz-red-light)');c.setAttribute('stroke-width','3');c.setAttribute('stroke-dasharray','3 3');cap.appendChild(c);const t=document.createElementNS(NS,'text');t.setAttribute('x',0);t.setAttribute('y',4);t.setAttribute('text-anchor','middle');t.setAttribute('font-size','11');t.setAttribute('font-weight','900');t.setAttribute('fill','var(--mz-red-light)');t.textContent='x';cap.appendChild(t);svg.appendChild(cap);}if(layout.branchPathD){addPath(layout.branchPathD,'var(--mz-red-light)',16,'',0.22);addPath(layout.branchPathD,'var(--mz-red-light)',3,'4 6',0.85);}(layout.branchBlockers||[]).forEach(addBlockCap);const R=30;layout.nodes.forEach(node=>{const vocab=mzkNodeVocab(node),isStart=node.kind==='path'&&node.index===0,isEnd=node.kind==='path'&&node.index===MZK_PATH.length-1;const g=document.createElementNS(NS,'g');g.setAttribute('class','mzk-node'+(node.kind==='branch'?' branch':'')+(isStart?' start':'')+(isEnd?' end':''));g.setAttribute('transform','translate('+node.x+','+node.y+')');const circle=document.createElementNS(NS,'circle');circle.setAttribute('class','mzk-node-circle');circle.setAttribute('r',R);g.appendChild(circle);if(vocab.image_url){const img=document.createElementNS(NS,'image');img.setAttributeNS('http://www.w3.org/1999/xlink','href',vocab.image_url);img.setAttribute('href',vocab.image_url);img.setAttribute('x',-R+5);img.setAttribute('y',-R+5);img.setAttribute('width',(R-5)*2);img.setAttribute('height',(R-5)*2);img.setAttribute('clip-path','circle('+(R-5)+'px)');img.setAttribute('preserveAspectRatio','xMidYMid slice');g.appendChild(img);}if(vocab.word){const label=document.createElementNS(NS,'text');label.setAttribute('class','mzk-node-label');label.setAttribute('x',0);label.setAttribute('y',R+14);label.setAttribute('text-anchor','middle');label.textContent=vocab.word;g.appendChild(label);}const bc=document.createElementNS(NS,'circle');bc.setAttribute('class','mzk-node-badge'+(node.kind==='branch'?' mzk-node-badge-dead':''));bc.setAttribute('cx',R-7);bc.setAttribute('cy',-R+7);bc.setAttribute('r',11);g.appendChild(bc);const bt=document.createElementNS(NS,'text');bt.setAttribute('class','mzk-node-badge-text');bt.setAttribute('x',R-7);bt.setAttribute('y',-R+11);bt.setAttribute('text-anchor','middle');bt.textContent=node.kind==='path'?String(node.index+1):'x';g.appendChild(bt);if(isStart||isEnd){const flag=document.createElementNS(NS,'text');flag.setAttribute('class','mzk-node-flag');flag.setAttribute('x',0);flag.setAttribute('y',-R-11);flag.setAttribute('text-anchor','middle');flag.setAttribute('fill',isStart?'var(--mz-orange)':'var(--mz-green)');flag.textContent=isStart?'START':'GOAL';g.appendChild(flag);}g.addEventListener('click',()=>mzkHandleTap(node,g));svg.appendChild(g);});const wrap=document.getElementById('mzkMazeWrap');wrap.innerHTML='';wrap.appendChild(svg);}
