...
(function () {
  ...
  const VOICES = {
    josh: {label: 'Josh', id: 'nzFihrBIvB34imQBuxub'},
    lily: {label: 'Lily', id: 'NoOVOzCQFLOvtsMoNcdT'},
    candy: {label: 'Candy', id: 'Nggzl2QAXh3OijoXD116'}
  };
  const AV = {
  ...
  function voiceIdToLabel(id) {
    const found = Object.keys(VOICES).find(k => VOICES[k].id === id);
    return found || 'josh';
  }
  function voiceSelectHtml() {
    const current = voiceIdToLabel(scene.teacherVoiceId);
    return '<label class="rk-label">Listen voice - ElevenLabs</label><select class="rk-select" data-voice-select="1">' +
      Object.keys(VOICES).map(k => '<option value="' + h(VOICES[k].id) + '" ' + (current === k ? 'selected' : '') + '>' + h(VOICES[k].label) + '</option>').join('') +
      '</select>';
  }
  ...
  async function speakAgent(i) {
    ...
    try {
      ...
      activeAudio.onended = activeAudio.onerror = function () {
      ...
      console.error('[roleplay kids] ElevenLabs listen failed', err);
      if (btn) {
        btn.classList.remove('speaking');
        btn.textContent = '🔊 Listen';
      }
  ...
  root.addEventListener('change', function (e) {
    let el = e.target;
    if (el.dataset.voiceSelect) {
      scene.teacherVoiceId = el.value;
      render();
    }
  });
