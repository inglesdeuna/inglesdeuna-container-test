...
function dot_to_dot_default_label_settings(): array {
    return [
        'mode' => 'number', // number, letter, word
        'start' => 1,
        'step' => 1,
        'end' => 20,
    ];
...
function dot_to_dot_normalize_label_settings($settings, int $pointsCount = 0): array {
    $d = dot_to_dot_default_label_settings();
    $s = is_array($settings) ? $settings : [];
    $mode = isset($s['mode']) && in_array($s['mode'], ['number','letter','word']) ? $s['mode'] : $d['mode'];
    $start = isset($s['start']) && (int)$s['start'] > 0 ? (int)$s['start'] : $d['start'];
    $step = isset($s['step']) && (int)$s['step'] > 0 ? (int)$s['step'] : $d['step'];
    $end = isset($s['end']) && (int)$s['end'] >= $start ? (int)$s['end'] : ($pointsCount > 0 ? $start + ($pointsCount-1)*$step : $d['end']);
    return [ 'mode'=>$mode, 'start'=>$start, 'step'=>$step, 'end'=>$end ];
}

function dot_to_dot_label_value(array $settings, int $index): int {
...
function dot_to_dot_number_to_letters(int $value): string {
    ...
    return $letters ?: (string)$value;
}

function dot_to_dot_number_to_words(int $value): string {
    ...
    if ($value < 1000) {
        ...
        $rest = $value % 100;
        return $ones[$hundred] . ' hundred' . ($rest ? ' ' . dot_to_dot_number_to_words($rest) : '');
    }
    return (string)$value;
}
...
function dot_to_dot_label_text(array $settings, int $index): string {
    ...
    $value = dot_to_dot_label_value($settings, $index);
    if ($mode === 'letter') return dot_to_dot_number_to_letters($value);
    if ($mode === 'word') return dot_to_dot_number_to_words($value);
    return (string)$value;
}

function dot_to_dot_apply_labels(array $points, array $settings): array {
    $labeled = [];
    foreach (array_values($points) as $i => $pt) {
        if (!is_array($pt) || !isset($pt['x'], $pt['y'])) continue;
        $x = (float)$pt['x'];
        $y = (float)$pt['y'];
        ...
        $labeled[] = [
            'x' => round($x, 6),
            'y' => round($y, 6),
            'label' => dot_to_dot_label_text($settings, $i)
        ];
    }
    return $labeled;
}

function dot_to_dot_normalize_payload($raw): array {
    $d = [
        'title' => dot_to_dot_default_title(),
        'instruction' => dot_to_dot_default_instruction(),
        'image' => '',
    ...
    $settings = dot_to_dot_normalize_label_settings($data['label_settings'] ?? [], isset($data['points']) ? count($data['points']) : 0);
    $points = isset($data['points']) && is_array($data['points']) ? dot_to_dot_apply_labels($data['points'], $settings) : [];
    return [
        'title' => trim($data['title'] ?? $d['title']) ?: $d['title'],
        'instruction' => trim($data['instruction'] ?? $d['instruction']) ?: $d['instruction'],
...
// DB helpers (assume $pdo is available)
function dot_to_dot_load_activity(PDO $pdo, string $unit, string $activityId): array {
    $stmt = $pdo->prepare("SELECT id, data FROM activities WHERE (id = :id OR (unit = :unit AND type = 'dot_to_dot')) ORDER BY id ASC LIMIT 1");
    $stmt->execute(['id'=>$activityId, 'unit'=>$unit]);
    $row = $stmt->fetch(PDO::FETCH_ASSOC);
    if (!$row) return dot_to_dot_normalize_payload(null);
    return dot_to_dot_normalize_payload($row['data'] ?? null);
}

function dot_to_dot_save_activity(PDO $pdo, string $unit, string $activityId, string $title, string $instruction, string $image, array $points, array $labelSettings): string {
    $payload = [
        'title' => $title,
    ...
    if ($activityId) {
        ...
        $stmt->execute(['data'=>$json, 'id'=>$activityId]);
        return $activityId;
    } else {
        $stmt = $pdo->prepare("INSERT INTO activities (unit, type, data) VALUES (:unit, 'dot_to_dot', :data) RETURNING id");
        $stmt->execute(['unit'=>$unit, 'data'=>$json]);
