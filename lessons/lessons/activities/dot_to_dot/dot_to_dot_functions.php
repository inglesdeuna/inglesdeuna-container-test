...
function normalize_dot_to_dot_payload($rawData): array
{
    ...
    $points = array();
    foreach ($pointsSource as $index => $point) {
        if (!is_array($point)) {
            continue;
        }
...
function save_dot_to_dot_activity(PDO $pdo, string $unit, string $activityId, string $title, string $instruction, string $image, array $points): string
{
    $columns = dot_to_dot_activities_columns($pdo);
