...
const ttsUrl = String(window.UNSCRAMBLE_TTS_URL || 'tts.php');
const quizMode = !!window.UNSCRAMBLE_QUIZ_MODE;
const quizSubmit = typeof window.UNSCRAMBLE_QUIZ_SUBMIT === 'function' ? window.UNSCRAMBLE_QUIZ_SUBMIT : null;
...
const quizSkipBtn = document.getElementById('us2QuizSkip');
...
function loadSentence(){
    ...
    if (nextBtn) nextBtn.textContent = quizMode ? 'Next' : (index >= sentences.length - 1 ? 'Finish' : 'Next');
...
function next(){
    if (quizMode) {
        stopAudio();
        if (quizSubmit) quizSubmit(builtWords().join(' '), false);
        return;
    }
    if (!locked) {
