...
.ddk-btn-next { background: #F97316; }
...
<div class="ddk-stage" id="ddkStage">
    ...
    <div id="ddkZoomTarget" class="ddk-zoom-target">
        ...
        <div class="ddk-controls" id="ddkControls">
            <?php if ($ddkQuizMode): ?>
            <button class="ddk-btn ddk-btn-next" type="button" onclick="submitQuizAnswer(false)">Next →</button>
            <button class="ddk-btn ddk-btn-show" type="button" onclick="submitQuizAnswer(true)">Skip</button>
            <?php else: ?>
...
const DDK_QUIZ_MODE   = <?= $ddkQuizMode ? 'true' : 'false' ?>;
...
function buildBank() {
    bank.innerHTML = '';
    shuffle(DDK_PAIRS).forEach(function(p) { bank.appendChild(createBankChip(p)); });
}
function clearQuizZone(zone) {
    if (!DDK_QUIZ_MODE || !zone || !zone.classList.contains('filled')) return;
    const answerId = zone.dataset.answerId || '';
    const pair = DDK_PAIRS.find(function(p) { return String(p.id) === String(answerId); });
    if (pair) bank.appendChild(createBankChip(pair));
    zone.classList.remove('filled', 'wrong');
    zone.textContent = '';
    zone.dataset.answerId = '';
}
...
function setupZones() {
    document.querySelectorAll('.ddk-zone').forEach(function(zone) {
        ...
        zone.addEventListener('pointerup', function(e) {
            ...
            if (DDK_QUIZ_MODE && zone.classList.contains('filled')) {
                clearQuizZone(zone);
                e.stopPropagation();
            }
        });
...
function handleDrop(zone, chipId, chipEl) {
    if (DDK_QUIZ_MODE) {
        if (zone.classList.contains('filled')) clearQuizZone(zone);
        zone.classList.add('filled');
        zone.classList.remove('wrong');
        zone.textContent = chipEl.textContent;
        zone.dataset.answerId = String(chipId);
        chipEl.remove();
        return;
    }
...
/* ── Quiz submission ───────────────────────────────────── */
function submitQuizAnswer(skipped) {
    if (!DDK_QUIZ_MODE || done || typeof window.DDK_QUIZ_SUBMIT !== 'function') return;
    done = true;
    const placements = {};
    document.querySelectorAll('.ddk-zone').forEach(function(zone) {
        if (zone.dataset.answerId) placements[String(zone.dataset.id)] = String(zone.dataset.answerId);
    });
    window.DDK_QUIZ_SUBMIT(placements, !!skipped);
}
