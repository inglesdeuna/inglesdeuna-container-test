...
async function generateAudio(item,btn){ if(!item.name){alert('Enter a name first.'); return;} btn.classList.add('loading'); btn.innerHTML='<i class="fas fa-spinner fa-spin"></i> Generating...'; try{ const res=await fetch('../../core/tts.php',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({text:item.name,voice:'Rachel'})}); const data=await res.json(); if(data.url){item.audio=data.url; btn.className='lc-audio-btn ready'; btn.innerHTML='<i class="fas fa-check"></i> Ready';}else throw new Error(data.error||'TTS failed'); }catch(e){btn.className='lc-audio-btn'; btn.innerHTML='<i class="fas fa-volume-up"></i> Generate'; alert('Audio generation failed: '+e.message);} }
