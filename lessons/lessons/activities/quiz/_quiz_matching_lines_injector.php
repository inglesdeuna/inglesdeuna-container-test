...
<div class="qzml-col">
<?php foreach($pairs as$i=>$pair):?><button type="button" draggable="true" class="qzml-card" data-side="left" data-left="<?=$i?>"><?=qzml_media($pair['left']??'',$pair['left_text']??'','Left item '.($i+1))?><span class="qzml-anchor"></span></button><?php endforeach;?>
...
var pairs=<?=json_encode(array_map(fn($p)=>(string)($p['right']??''),$pairs),JSON_UNESCAPED_UNICODE|JSON_UNESCAPED_SLASHES)?>,selected=null,dragged=null,links={};
...
lefts.forEach(function(el){el.addEventListener('click',function(){selected=el.dataset.left;paint();});el.addEventListener('dragstart',function(){dragged=el.dataset.left;});});
rights.forEach(function(el){el.addEventListener('click',function(){if(selected!==null)connect(selected,el.dataset.right);});el.addEventListener('dragover',function(e){e.preventDefault();});el.addEventListener('drop',function(e){e.preventDefault();if(dragged!==null)connect(dragged,el.dataset.right);dragged=null;});});
