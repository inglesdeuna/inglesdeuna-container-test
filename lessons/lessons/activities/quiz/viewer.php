...
function qz_is_scoreable_type($type){return in_array($type,['quiz','multiple_choice','video_comprehension','fillblank','fill_blank','fill_in_blank','writing_practice','question_answer','matching_lines','match','dictation','pronunciation','unscramble','listen_order','drag_drop','drag_drop_kids'],true);}
...
$qzAllAttempts=(($mode==='result'||$mode==='review')&&$qzHasDb)?qz_load_all_completed_attempts($pdo,$qzStudentId,$unitId,$assignment):[];
...
$quizStartHref='?mode=quiz&q=0&unit='.$unitId.'&assignment='.$assignment.$rtParam;$qzShowTakeQuizState=in_array($mode,['result','review'],true)&&!$qzHasFirstAttemptCompleted;$resultTabHref=$qzHasFirstAttemptCompleted?$resultHref:$quizStartHref;$reviewTabHref=$qzHasFirstAttemptCompleted?$reviewHref:$quizStartHref;
