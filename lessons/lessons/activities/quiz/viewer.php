...
$unitId=isset($_GET['unit'])?intval($_GET['unit']):0;$returnTo=isset($_GET['return_to'])?trim((string)$_GET['return_to']):'';$assignment=qz_resolve_assignment_id($_GET['assignment']??0,$returnTo);$mode=$_GET['mode']??'intro';$qIndex=isset($_GET['q'])?intval($_GET['q']):0;$qzTeacherContext=!empty($GLOBALS['qzTeacherContext']);$qzTeacherId=$qzTeacherContext?trim((string)($GLOBALS['qzTeacherId']??'')):'';if(!$unitId)die('Missing unit id.');$pdo=get_pdo();$st=$pdo->prepare('SELECT * FROM activities WHERE unit_id=:u ORDER BY id ASC');$st->execute(['u'=>$unitId]);$activities=$st->fetchAll(PDO::FETCH_ASSOC);$all=[];foreach($activities as$act)foreach(qz_normalize_activity($act)as$q)$all[]=$q;if(!$all)die('<div style="font-family:Arial;padding:30px;color:#7c3aed"><h2>No scoreable quiz activities found for this unit.</h2></div>');
