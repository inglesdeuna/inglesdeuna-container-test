...
$unitId=isset($_GET['unit'])?intval($_GET['unit']):0;$returnTo=isset($_GET['return_to'])?trim((string)$_GET['return_to']):'';$assignment=qz_resolve_assignment_id($_GET['assignment']??0,$returnTo);$mode=$_GET['mode']??'intro';$qIndex=isset($_GET['q'])?intval($_GET['q']):0;$qzTeacherContext=!empty($GLOBALS['qzTeacherContext']);$qzTeacherId=$qzTeacherContext?trim((string)($GLOBALS['qzTeacherId']??'')):'';if(!$unitId)die('Missing unit id.');$pdo=get_pdo();$st=$pdo->prepare('SELECT * FROM activities WHERE unit_id=:u ORDER BY id ASC');$st->execute(['u'=>$unitId]);$activities=$st->fetchAll(PDO::FETCH_ASSOC);$qzQuizTitle='Unit Quiz';$qzQuizDescription='';foreach($activities as$qzAct){if(strtolower(trim((string)($qzAct['type']??'')))==='quiz'){$qzActData=json_decode((string)($qzAct['data']??''),true);if(is_array($qzActData)){$qzT=trim((string)($qzActData['title']??''));$qzD=trim((string)($qzActData['description']??''));if($qzT!=='')$qzQuizTitle=$qzT;if($qzD!=='')$qzQuizDescription=$qzD;}break;}}$all=[];foreach($activities as$act)foreach(qz_normalize_activity($act)as$q)$all[]=$q;if(!$all)die('<div style="font-family:Arial;padding:30px;color:#7c3aed"><h2>No scoreable quiz activities found for this unit.</h2></div>');
if(!$qzTeacherContext&&((!isset($_SESSION['student_logged'])||$_SESSION['student_logged']!==true)||trim((string)($_SESSION['student_id']??''))==='')){header('Location: ../../academic/login_student.php');exit;}
$qzStudentId=$qzTeacherContext?'':trim((string)($_SESSION['student_id']??''));$qzHasDb=!$qzTeacherContext&&$qzStudentId!=='';if($qzHasDb){qz_ensure_quiz_state_table($pdo);qz_recalculate_student_quiz_scores($pdo,$qzStudentId,(string)$assignment,(string)$unitId);}$qzDbState=$qzHasDb?qz_load_db_state($pdo,$qzStudentId,$unitId,$assignment):null;error_log("UNIT: ".$unitId);error_log("ASSIGNMENT: ".$assignment);error_log("STUDENT: ".$qzStudentId);error_log("ATTEMPT FOUND: ".json_encode($qzDbState,JSON_UNESCAPED_UNICODE|JSON_UNESCAPED_SLASHES));
$qzSessionPrefix=$qzTeacherContext?'qz_teacher_'.md5($qzTeacherId.'|'.$assignment).'_':'';$attKey=$qzSessionPrefix.'qz_attempt_'.$unitId.'_'.$assignment;if(!isset($_SESSION[$attKey])){if($qzDbState!==null)$_SESSION[$attKey]=qz_bool($qzDbState['is_completed'])?(int)$qzDbState['attempt_number']+1:max(1,(int)$qzDbState['attempt_number']);else $_SESSION[$attKey]=1;}$att=(int)$_SESSION[$attKey];$setKey=$qzSessionPrefix.'qz_set_'.$unitId.'_'.$assignment.'_'.$att;$ansKey=$qzSessionPrefix.'qz_answers_'.$unitId.'_'.$assignment.'_'.$att;
if(isset($_GET['reset'])){$qzCanReset=true;if($qzHasDb){$qzResetAttempts=qz_load_all_completed_attempts($pdo,$qzStudentId,$unitId,$assignment);$qzHasAttempt2=false;foreach($qzResetAttempts as$qzResetAttempt){if((int)($qzResetAttempt['attempt_number']??0)>=2){$qzHasAttempt2=true;break;}}$qzResetMetrics=qz_load_unit_metrics($pdo,$qzStudentId,$unitId,$assignment);$qzResetQuiz=max(0,min(100,(float)($qzResetMetrics['quiz_score_percent']??0)));$qzResetUnit=max(0,min(100,(float)($qzResetMetrics['completion_percent']??0)));$qzResetCombined=($qzResetUnit*0.6)+($qzResetQuiz*0.4);$qzCanReset=($qzResetQuiz<=64||$qzResetCombined<=64)&&!$qzHasAttempt2;}if($qzCanReset){$_SESSION[$attKey]=2;$qzRetrySetKey=$qzSessionPrefix.'qz_set_'.$unitId.'_'.$assignment.'_2';$qzRetryAnswersKey=$qzSessionPrefix.'qz_answers_'.$unitId.'_'.$assignment.'_2';unset($_SESSION[$qzRetrySetKey],$_SESSION[$qzRetryAnswersKey]);if($qzHasDb)qz_clear_teacher_unlock($pdo,$qzStudentId,$unitId,$assignment);qz_redirect('intro',$unitId,$assignment,null,['return_to'=>$returnTo]);}qz_redirect('result',$unitId,$assignment,null,['return_to'=>$returnTo]);}
...
if(empty($_SESSION[$ansKey])&&!qz_quiz_within_limits((array)$_SESSION[$setKey]))$_SESSION[$setKey]=qz_build($all,$unitId,$assignment,$att);
$quiz=$_SESSION[$setKey];$qzInstructionById=[];foreach($all as$qzSourceQuestion){$qzSourceId=(string)($qzSourceQuestion['id']??'');$qzSourceInstruction=trim((string)($qzSourceQuestion['activity_instruction']??''));if($qzSourceId!==''&&$qzSourceInstruction!=='')$qzInstructionById[$qzSourceId]=$qzSourceInstruction;}$qzInstructionsUpdated=false;foreach($quiz as&$qzStoredQuestion){$qzStoredId=(string)($qzStoredQuestion['id']??'');if($qzStoredId!==''&&isset($qzInstructionById[$qzStoredId])&&trim((string)($qzStoredQuestion['activity_instruction']??''))===''){$qzStoredQuestion['activity_instruction']=$qzInstructionById[$qzStoredId];$qzInstructionsUpdated=true;}}unset($qzStoredQuestion);if($qzInstructionsUpdated)$_SESSION[$setKey]=$quiz;$answers=&$_SESSION[$ansKey];$total=count($quiz);$returnHref=$returnTo!==''?$returnTo:'../../academic/student_dashboard.php';
if($_SERVER['REQUEST_METHOD']==='POST'&&$mode==='quiz'){$qIndex=max(0,min($qIndex,$total-1));if(isset($answers[$qIndex])){$la=count($answers)>0?max(array_keys($answers)):-1;$nu=max(0,min($total-1,$la+1));if(count($answers)>=$total)qz_redirect('result',$unitId,$assignment,null,['return_to'=>$returnTo]);else qz_redirect('quiz',$unitId,$assignment,$nu,['return_to'=>$returnTo]);}$q=$quiz[$qIndex];if(isset($_POST['skip'])){$score=qz_answer_score($q,null);$answers[$qIndex]=['answer'=>null,'skipped'=>true,'correct'=>false,'earned'=>0.0,'possible'=>$score['possible']];}else{$ans=in_array($q['type'],['match','drag_drop'],true)?(isset($_POST['answer'])&&is_array($_POST['answer'])?$_POST['answer']:[]):($_POST['answer']??'');$score=qz_answer_score($q,$ans);$answers[$qIndex]=['answer'=>$ans,'skipped'=>false,'correct'=>$score['correct'],'earned'=>$score['earned'],'possible'=>$score['possible']];}if($qIndex>=$total-1){foreach($quiz as$qzMissingIndex=>$qzMissingQuestion){if(array_key_exists($qzMissingIndex,$answers))continue;$qzMissingScore=qz_answer_score($qzMissingQuestion,null);$answers[$qzMissingIndex]=['answer'=>null,'skipped'=>true,'correct'=>false,'earned'=>0.0,'possible'=>$qzMissingScore['possible']];}ksort($answers);}$stats=qz_answers_totals($quiz,$answers);$qzC=(int)$stats['correct_questions'];$qzS=(int)$stats['skipped_questions'];$qzTotal=(int)$stats['possible'];$qzW=max(0,$qzTotal-$qzC-$qzS);$qzP=$qzTotal?round($stats['earned']/$qzTotal*100):0;$qzDone=count($answers)>=$total;if($qzHasDb){qz_save_db_state($pdo,$qzStudentId,$unitId,$assignment,$att,$quiz,$answers,$qzDone,(int)$qzP,$qzC,$qzW,$qzS,$qzTotal);if($qzDone)qz_save_quiz_unit_score($pdo,$qzStudentId,$unitId,$assignment,(int)$qzP,$att);}$n=$qIndex+1;qz_redirect($n>=$total?'result':'quiz',$unitId,$assignment,$n>=$total?null:$n,['return_to'=>$returnTo]);}
$labels=['dictation'=>['Dictation','Listen and type what you hear','ti-keyboard'],'pronunciation'=>['Pronunciation','Say the phrase','ti-microphone'],'multiple_choice'=>['Multiple choice','Pick the correct answer','ti-checks'],'fill'=>['Fill in the blank','Complete the sentence','ti-pencil'],'writing_practice'=>['Writing practice','Write your answer','ti-writing'],'match'=>['Match','Connect each word to its pair','ti-arrows-shuffle'],'drag_drop'=>['Drag and drop','Arrange or match items','ti-hand-move'],'drag_drop_kids'=>['Drag and Drop Kids','Place each label on the picture','ti-hand-click'],'unscramble'=>['Unscramble','Put the words in order','ti-sort-ascending'],'unscramble_kids'=>['Unscramble Kids','Look at the picture and spell the word','ti-abc']];$counts=[];foreach($quiz as$q){$bt=$q['block_type']??$q['type'];$counts[$bt]=($counts[$bt]??0)+1;}$score_stats=qz_answers_totals($quiz,$answers);$correct=(int)$score_stats['correct_questions'];$skip=(int)$score_stats['skipped_questions'];$score_total=(int)$score_stats['possible'];$wrong=max(0,$score_total-$correct-$skip);$percent=$score_total?round($score_stats['earned']/$score_total*100):0;
...
if($qzTeacherContext){try{$qzTeacherScoreStmt=$pdo->prepare("SELECT AVG(percent) FROM teacher_activity_results WHERE teacher_id=:teacher AND assignment_id=:assignment AND unit_id=:unit AND activity_id<>'unit_quiz'");$qzTeacherScoreStmt->execute(['teacher'=>$qzTeacherId,'assignment'=>(string)$assignment,'unit'=>(string)$unitId]);$qzTeacherActivityAvg=$qzTeacherScoreStmt->fetchColumn();if($qzTeacherActivityAvg!==false&&$qzTeacherActivityAvg!==null)$unit_score=max(0,min(100,(float)$qzTeacherActivityAvg));}catch(Throwable $e){}try{$qzTeacherUnitStmt=$pdo->prepare("SELECT COALESCE(u.name,'Unit '||u.id::text) AS unit_title,COALESCE(t.name,'Teacher') AS teacher_name FROM units u LEFT JOIN teacher_assignments ta ON ta.id=:assignment LEFT JOIN teachers t ON t.id=ta.teacher_id WHERE u.id::text=:unit LIMIT 1");$qzTeacherUnitStmt->execute(['assignment'=>(string)$assignment,'unit'=>(string)$unitId]);$qzTeacherCtx=$qzTeacherUnitStmt->fetch(PDO::FETCH_ASSOC);if(is_array($qzTeacherCtx)){$unit_title=trim((string)($qzTeacherCtx['unit_title']??''))?:$unit_title;$teacher_name=trim((string)($qzTeacherCtx['teacher_name']??''))?:$teacher_name;}}catch(Throwable $e){}$phase_avg=round(($unit_score*0.6)+($quiz_score*0.4),1);}
  ...
  <div style="max-width:760px;margin:0 auto;display:flex;flex-direction:column;gap:16px;">
    <div style="background:#fff;border-radius:24px;border:1px solid #EDE9FA;padding:24px;box-shadow:0 4px 32px rgba(127,119,221,.10);text-align:center;">
      ...
      <div style="display:flex;justify-content:center;gap:32px;align-items:center;margin-bottom:16px;flex-wrap:wrap;">
        ...
        <?= result_ring($unit_final,'#1D9E75','Total score') ?>
      </div>
