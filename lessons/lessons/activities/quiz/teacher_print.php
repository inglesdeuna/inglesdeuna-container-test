...
$s->execute(['u'=>$unitId]);
$tpActivities=$s->fetchAll(PDO::FETCH_ASSOC)?:[];
$tpQuizDescription='';
foreach($tpActivities as$tpAct){if(strtolower(trim((string)($tpAct['type']??'')))==='quiz'){$tpActData=json_decode((string)($tpAct['data']??''),true);if(is_array($tpActData)){$tpD=trim((string)($tpActData['description']??''));if($tpD!=='')$tpQuizDescription=$tpD;}break;}}
$all=[];
foreach($tpActivities as $activity){foreach(qz_normalize_activity($activity) as $q){if(($q['type']??'')!=='pronunciation')$all[]=$q;}}
