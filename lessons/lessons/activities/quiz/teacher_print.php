...
function tp_body(array $q): string{
    ...
    if($type==='multiple_choice'){
        ...
        $o='<div class="opts'.($isImage?' image-options':'').'">';
        foreach((array)($q['options']??[]) as $i=>$v){
            $label=chr(65+$i).'.';
            $content=($isImage||tp_is_image($v))?tp_media($v,'Option '.$label):tp_h($v);
            $o.='<div class="opt"><b>'.$label.'</b><div class="opt-content">'.$content.'</div></div>';
        }
        return $o.'</div>';
    }
    if($type==='match'){
    ...
    if($type==='drag_drop'){$count=max(1,count((array)($q['correct_words']??[])));return'<div class="words">'.str_repeat('<span></span>',$count).'</div>';}
