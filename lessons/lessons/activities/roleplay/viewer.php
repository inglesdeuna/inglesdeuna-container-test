...
function render(){if(!root)return;const sc=root.querySelector('.rp-scroll,.rp-editor-body');const top=sc?sc.scrollTop:0;root.innerHTML=view==='editor'?editor():player();const sc2=root.querySelector('.rp-scroll,.rp-editor-body');if(sc2)sc2.scrollTop=top}
function submit(){if(!activeInput.trim()){alert('Please say or type your answer first.');return}answers[completed]=activeInput.trim();scores[completed]=grade(activeInput,turns[completed]?turns[completed].ideal:'');completed=Math.min(turns.length,completed+1);activeInput='';render()}
