...
(function(){
const root=document.getElementById('roleplay-root');let scene=normScene(window.RP_SAVED_SCENE||{});let turns=normTurns(window.RP_SAVED_TURNS||[]);let view=window.RP_ALLOW_EDITOR?'editor':'player';let completed=0;let answers=[];let scores=[];let shownAnswers=[];let activeInput='';let status='';let saving=false;
...
function render(){if(!root)return;const sc=root.querySelector('.rp-scroll,.rp-editor-body');const top=sc?sc.scrollTop:0;root.innerHTML=view==='editor'?editor():(view==='complete'?completedPage():player());const sc2=root.querySelector('.rp-scroll,.rp-editor-body');if(sc2)sc2.scrollTop=top}
function nextTurn(){const i=completed;if(!activeInput.trim()&&!shownAnswers[i]){alert('Please say/type your answer or tap Show answer first.');return}answers[i]=activeInput.trim()||'(Used Show answer)';scores[i]=scoreTurn(activeInput,turns[i]?turns[i].ideal:'');completed=Math.min(turns.length,completed+1);activeInput='';if(completed>=turns.length)view='complete';render()}
function startMic(i){const SR=window.SpeechRecognition||window.webkitSpeechRecognition;if(!SR){alert('Speech recognition is not supported in this browser. You can type your answer.');return}const rec=new SR();rec.lang='en-US';rec.continuous=false;rec.interimResults=false;const btn=root.querySelector('[data-action="mic"][data-index="'+i+'"]');const box=root.querySelector('[data-answer="1"]');if(btn){btn.classList.add('listening');btn.textContent='🎙 Listening...'}rec.onresult=e=>{activeInput=e.results[0][0].transcript;if(box)box.value=activeInput};rec.onerror=()=>{if(btn){btn.classList.remove('listening');btn.textContent='🎙 Now say it'}};rec.onend=()=>{if(btn){btn.classList.remove('listening');btn.textContent='🎙 Now say it'}};rec.start()}
root.addEventListener('input',e=>{const el=e.target;if(el.dataset.scene){scene[el.dataset.scene]=el.value;render();return}if(el.dataset.turn){const i=Number(el.dataset.turn);turns[i]=Object.assign({},turns[i],{[el.dataset.prop]:el.value});render();return}if(el.dataset.answer){activeInput=el.value}});
root.addEventListener('click',async e=>{const btn=e.target.closest('[data-action]');if(!btn)return;e.preventDefault();const a=btn.dataset.action;if(a==='mic')startMic(Number(btn.dataset.index));if(a==='show-answer'){shownAnswers[completed]=true;render()}if(a==='next')nextTurn();if(a==='restart'){view='player';completed=0;answers=[];scores=[];shownAnswers=[];activeInput='';render()}if(a==='preview'){view='player';completed=0;activeInput='';answers=[];scores=[];shownAnswers=[];render()}if(a==='add-turn'){turns.push({id:uid(),agent:'',hint:'',ideal:'',criteria:''});render()}if(a==='remove-turn'){turns=turns.filter((_,i)=>i!==Number(btn.dataset.index));if(!turns.length)turns=normTurns([]);render()}if(a==='save'){if(!window.RP_ACTIVITY_ID){status='No activity ID - cannot save.';render();return}saving=true;status='Saving...';render();try{const resp=await fetch('save.php',{method:'POST',headers:{'Content-Type':'application/json'},credentials:'same-origin',body:JSON.stringify({id:window.RP_ACTIVITY_ID,scene:scene,turns:turns})});const json=await resp.json().catch(()=>({}));if(!resp.ok||!json.ok)throw new Error(json.error||('HTTP '+resp.status));status='Saved successfully'}catch(err){status='Could not save: '+err.message}finally{saving=false;render()}}});try{render()}catch(err){console.error('[roleplay] render error',err);root.innerHTML='<div style="padding:20px">Roleplay could not render. Check console.</div>'}
})();
</script>
<?php
