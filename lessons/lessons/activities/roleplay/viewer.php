...
if ($activityId !== '') {
  ...
  error_log('[roleplay/viewer] Viewer loaded data ' . json_encode([
...
// Backward compatibility: normalize scene fields.
...
<script>
window.RP_ACTIVITY_ID  = <?= json_encode($activityId) ?>;
window.RP_RETURN_TO    = <?= json_encode($returnTo) ?>;
window.RP_SAVED_SCENE  = <?= json_encode($savedScene) ?>;
window.RP_SAVED_TURNS  = <?= json_encode($savedTurns) ?>;
window.RP_ALLOW_EDITOR = <?= json_encode($allowEditor) ?>;
window.RP_START_VIEW   = <?= json_encode($startView) ?>;
</script>
...
function computeRoleplayCompletionSummary(turns, results) {
  ...
  return { totalBlocks, completedBlocks, correctBlocks, wrongBlocks, scorePct };
}
...
function buildActivitySaveUrl(returnTo, activityId, activityType, percent, errors, total) {
  ...
  return safeReturnTo
    ...
    + "&activity_type=" + encodeURIComponent(String(activityType || "roleplay"));
}
...
async function fetchElevenLabsAudioBlob(text, voiceId) {
  ...
  let res;
  try {
    res = await fetch("tts.php", { method: "POST", body: fd, credentials: "same-origin", cache: "no-store" });
  } catch (err) {
    throw buildTtsError("Could not contact TTS endpoint", "elevenlabs_request_failed");
  }

  if (!res.ok) {
    let detail = `TTS error ${res.status}`;
    try { const j = await res.json(); if (j && typeof j.error === "string" && j.error.trim() !== "") detail = j.error; } catch (e) {}
...
async function playAudioBlob(blob, audioRef, hooks = {}) {
  ...
  if (audioRef && audioRef.current) { audioRef.current.pause(); audioRef.current = null; }
  ...
  await new Promise((resolve, reject) => {
    ...
    onPlaying();
    audio.onended = () => { URL.revokeObjectURL(url); if (audioRef) audioRef.current = null; onIdle(); resolve(); };
    audio.onerror = () => { URL.revokeObjectURL(url); if (audioRef) audioRef.current = null; onIdle(); reject(buildTtsError("Audio playback failed", "playback_failed")); };
    audio.play().catch(() => { URL.revokeObjectURL(url); if (audioRef) audioRef.current = null; onIdle(); reject(buildTtsError("Audio playback blocked", "playback_failed")); });
  });
}

const DEFAULT_SCENE = {
  title: "Roleplay", icon: "🎭", desc: "Practice speaking English!", agentName: "Teacher",
  agentRole: "Teacher", studentRole: "Student", sceneImage: "",
  teacherVoiceId: "nzFihrBIvB34imQBuxub",
};

// ── SHARED COMPONENTS ─────────────────────────────────────────
const Kicker = ({ children }) => (
  <span style={{ display: "inline-block", background: C.orangeLight, color: C.orange, fontSize: 11, fontWeight: 800, letterSpacing: ".07em", padding: "3px 12px", borderRadius: 20, textTransform: "uppercase", marginBottom: 12 }}>{children}</span>
);

const Card = ({ children, style = {} }) => (
  <div style={{ background: C.white, border: `1.5px solid ${C.cardBorder}`, borderRadius: 24, padding: 20, boxShadow: "0 4px 24px rgba(127,119,221,.10)", ...style }}>{children}</div>
);

const Btn = ({ children, onClick, color = C.orange, style = {}, disabled = false }) => (
  <button onClick={onClick} disabled={disabled} style={{ background: disabled ? C.purpleLight : color, color: disabled ? C.purpleSub : C.white, border: "none", borderRadius: 999, padding: "12px 0", width: "100%", fontFamily: "'Nunito', sans-serif", fontWeight: 800, fontSize: 14, cursor: disabled ? "not-allowed" : "pointer", display: "flex", alignItems: "center", justifyContent: "center", gap: 7, ...style }}>{children}</button>
);

const MiniLabel = ({ children, color = C.purpleSub }) => (
  <div style={{ fontSize: 10, fontWeight: 800, color, textTransform: "uppercase", letterSpacing: ".05em", marginBottom: 4 }}>{children}</div>
);

const Topbar = ({ title, right }) => (
  <div style={{ background: C.white, borderBottom: `1px solid #F0EEF8`, padding: "12px 20px", display: "flex", alignItems: "center", justifyContent: "space-between", position: "sticky", top: 0, zIndex: 100 }}>
    <span style={{ fontFamily: "'Fredoka', sans-serif", fontSize: 20, fontWeight: 600, color: C.orange }}>{title}</span>
    {right}
  </div>
);
  ...
  useEffect(() => {
    if (active && analyserRef?.current) {
      const draw = () => {
        ...
        barsRef.current.forEach((b, i) => { if (b) b.style.height = (6 + (data[i % data.length] || 0) / 255 * 32) + "px"; });
        frameRef.current = requestAnimationFrame(draw);
      };
      draw();
    } else {
      if (frameRef.current) cancelAnimationFrame(frameRef.current);
      barsRef.current.forEach((b) => { if (b) b.style.height = (7 + Math.random() * 14) + "px"; });
    }
    return () => { if (frameRef.current) cancelAnimationFrame(frameRef.current); };
  }, [active]);

  return (
    <div style={{ background: C.bg, borderRadius: 14, height: 52, display: "flex", alignItems: "center", justifyContent: "center", gap: 3, padding: "0 10px", overflow: "hidden" }}>
      {Array.from({ length: NUM }).map((_, i) => (
        <div key={i} ref={el => barsRef.current[i] = el} style={{ width: 4, height: 10 + Math.random() * 18, borderRadius: 2, background: active ? C.purple : C.purpleMid, transition: "height .08s" }} />
      ))}
    </div>
  );
};
...
function useRecorder() {
  ...
  const start = useCallback(async () => {
    try {
      ...
      mr.ondataavailable = (event) => { if (event.data && event.data.size > 0) audioChunksRef.current.push(event.data); };
      mr.onstop = () => {
      ...
      setupSR();
      if (recognitionRef.current) { recognitionRef.current._active = true; try { recognitionRef.current.start(); } catch (e) {} }
  ...
  const stop = useCallback(() => {
    ...
    setHasRecorded(true);
    if (recognitionRef.current) { recognitionRef.current._active = false; try { recognitionRef.current.stop(); } catch (e) {} }
    if (mediaRecRef.current?.state !== "inactive") mediaRecRef.current?.stop();
    if (audioCtxRef.current) { try { audioCtxRef.current.close(); } catch (e) {} audioCtxRef.current = null; analyserRef.current = null; }
  }, []);
  ...
  const reset = useCallback(() => {
    ...
    setFinalText(""); setInterimText(""); setRecSecs(0); setHasRecorded(false); setIsRecording(false); setRecordedAudioUrl("");
  ...
  return { isRecording, finalText, interimText, recSecs, hasRecorded, recordedAudioUrl, analyserRef, start, stop, reset, playRecording };
}

// ── SAVE ──────────────────────────────────────────────────────
async function saveActivity(scene, turns) {
  const id = window.RP_ACTIVITY_ID;
  ...
  console.log("[roleplay] saveActivity called", { activityId: id, saveUrl: saveUrl.toString() });
  ...
  try {
    ...
    if (!r.ok) {
      ...
      console.error("[roleplay] saveActivity failed", { activityId: id, error: errorMsg });
      return { ok: false, error: errorMsg };
    }
    console.log("[roleplay] saveActivity success", { activityId: id });
    return data;
  } catch (e) {
    console.error("[roleplay] saveActivity exception", { activityId: id, error: String(e) });
...
const TEMPLATES = [
  ...
  { icon: "🏢", label: "Office",
    ...
    turns: [ { agent: "", hint: "", ideal: "", criteria: "" }, { agent: "", hint: "", ideal: "", criteria: "" } ],
  },
  { icon: "🎉", label: "Party",
...
function EditorView({ scene, turns, onSceneChange, onTurnsChange, onStart }) {
  ...
  const deleteTurn = i => { if (turns.length <= 1) return; const t = [...turns]; t.splice(i, 1); onTurnsChange(t); };
  const updateTurn = (i, key, val) => { const t = [...turns]; t[i] = { ...t[i], [key]: val }; onTurnsChange(t); };
  const moveTurnUp = i => { if (i === 0) return; const t = [...turns]; [t[i - 1], t[i]] = [t[i], t[i - 1]]; onTurnsChange(t); };
  const moveTurnDown = i => { if (i === turns.length - 1) return; const t = [...turns]; [t[i], t[i + 1]] = [t[i + 1], t[i]]; onTurnsChange(t); };
  const applyTemplate = tpl => { setActiveTemplate(tpl.label); onSceneChange({ ...tpl.scene }); onTurnsChange(tpl.turns.map(t => ({ ...t }))); };

  const handleSave = async () => {
    console.log("[roleplay] handleSave triggered", { activityId: window.RP_ACTIVITY_ID || null, turnsCount: Array.isArray(turns) ? turns.length : 0 });
  ...
  const handlePreviewTeacherVoice = useCallback(async () => {
    ...
    try {
      ...
      alert("Could not play voice preview: " + (e && e.message ? e.message : "Unknown error"));
    }
  }, [scene, turns]);

  useEffect(() => { return () => { if (previewAudioRef.current) { previewAudioRef.current.pause(); previewAudioRef.current = null; } }; }, []);

  const arrowBtnStyle = disabled => ({
    background: "none", border: `1.5px solid ${disabled ? C.purpleLight : C.purpleMid}`,
    borderRadius: 6, cursor: disabled ? "default" : "pointer",
    color: disabled ? C.purpleLight : C.purpleSub, fontSize: 10, padding: "2px 5px", lineHeight: 1, fontWeight: 800,
  });

  return (
    <div style={{ background: C.bg, minHeight: "100%" }}>
      <Topbar title="🎭 Roleplay" right={<span style={{ fontSize: 12, color: C.purpleSub, fontWeight: 700 }}>Activity Editor</span>} />
      <div style={{ maxWidth: 680, margin: "0 auto", padding: "20px 16px 60px" }}>
        <Kicker>Activity Builder</Kicker>

        <div style={{ marginBottom: 14 }}>
          <button onClick={() => setShowTemplates(v => !v)} style={{ background: "none", border: "none", cursor: "pointer", fontFamily: "'Nunito', sans-serif", fontWeight: 800, fontSize: 13, color: C.purple, display: "flex", alignItems: "center", gap: 6, padding: "4px 0" }}>
          ...
          {showTemplates && (
            <div style={{ display: "flex", flexWrap: "wrap", gap: 7, marginTop: 10 }}>
              {TEMPLATES.map(tpl => {
                ...
                return (
                  <button key={tpl.label} onClick={() => applyTemplate(tpl)} style={{ background: active ? C.purple : C.purpleLight, color: active ? C.white : C.purple, border: `1.5px solid ${C.cardBorder}`, borderRadius: 20, fontFamily: "'Nunito', sans-serif", fontSize: 12, fontWeight: 800, padding: "6px 14px", cursor: "pointer" }}>
                    {tpl.icon} {tpl.label}
                  </button>
                );
              })}
        ...
        <Card style={{ marginBottom: 16, borderRadius: 20 }}>
          <div style={{ fontFamily: "'Fredoka', sans-serif", fontSize: 16, fontWeight: 600, color: C.orange, marginBottom: 14 }}>🎭 Scene Setup</div>

          <div style={{ display: "grid", gridTemplateColumns: "1fr 72px", gap: 10, marginBottom: 10 }}>
            <div><MiniLabel>Activity title</MiniLabel><input value={scene.title} onChange={e => onSceneChange({ ...scene, title: e.target.value })} style={inputStyle} placeholder="e.g. At the Restaurant" /></div>
            <div><MiniLabel>Scene icon</MiniLabel><input value={scene.icon} onChange={e => onSceneChange({ ...scene, icon: e.target.value })} maxLength={2} style={inputStyle} placeholder="🎭" /></div>
          </div>

          <div style={{ marginBottom: 10 }}>
            <MiniLabel>Situation description</MiniLabel>
            <textarea value={scene.desc} onChange={e => onSceneChange({ ...scene, desc: e.target.value })} rows={3} placeholder={"Describe the situation the student is in.\ne.g. You are a customer at a busy café ordering lunch..."} style={{ ...inputStyle, resize: "none", lineHeight: 1.55 }} />
          </div>

          <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 10, marginBottom: 10 }}>
            <div><MiniLabel>Agent name</MiniLabel><input value={scene.agentName} onChange={e => onSceneChange({ ...scene, agentName: e.target.value })} style={inputStyle} placeholder="e.g. Maria, Dr. Smith" /></div>
            <div><MiniLabel>Agent role</MiniLabel><input value={scene.agentRole} onChange={e => onSceneChange({ ...scene, agentRole: e.target.value })} style={inputStyle} placeholder="e.g. Waiter, Doctor, Receptionist" /></div>
          </div>

          <div style={{ marginBottom: 10 }}>
            <MiniLabel>Student role</MiniLabel>
            <input value={scene.studentRole} onChange={e => onSceneChange({ ...scene, studentRole: e.target.value })} style={inputStyle} placeholder="e.g. Customer, Patient, Tourist" />
          </div>

          <div style={{ marginBottom: 10 }}>
            <MiniLabel>Scene image URL</MiniLabel>
            <input value={scene.sceneImage || ""} onChange={e => onSceneChange({ ...scene, sceneImage: e.target.value })} style={inputStyle} placeholder="https://... (optional)" />
          </div>
          ...
          <div>
            ...
            <div style={{ display: "flex", gap: 8, alignItems: "center", flexWrap: "wrap" }}>
              ...
              <button onClick={handlePreviewTeacherVoice} disabled={ttsPreviewState !== "idle"} style={{ background: ttsPreviewState !== "idle" ? "#C5C1ED" : C.purple, color: C.white, border: "none", borderRadius: 10, padding: "10px 14px", fontSize: 12, fontWeight: 800, cursor: ttsPreviewState !== "idle" ? "not-allowed" : "pointer", whiteSpace: "nowrap", flexShrink: 0 }}>
        ...
        {turns.map((t, i) => (
          <Card key={i} style={{ marginBottom: 10, borderRadius: 20 }}>
            <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", marginBottom: 12 }}>
              <span style={{ background: C.purpleLight, color: C.purple, fontSize: 11, fontWeight: 800, padding: "3px 10px", borderRadius: 20 }}>Turn {i + 1}</span>
              <div style={{ display: "flex", alignItems: "center", gap: 4 }}>
                ...
                {turns.length > 1 && (<button onClick={() => deleteTurn(i)} style={{ background: "none", border: "none", cursor: "pointer", color: "#ccc", fontSize: 15, padding: "2px 6px", borderRadius: 6, fontWeight: 800 }}>✕</button>)}
              </div>
            </div>
            <div style={{ display: "flex", flexDirection: "column", gap: 9 }}>
              <div><MiniLabel>Agent says</MiniLabel><textarea value={t.agent} onChange={e => updateTurn(i, "agent", e.target.value)} rows={2} placeholder="What the agent/character says to the student" style={{ ...inputStyle, resize: "none", lineHeight: 1.55 }} /></div>
              <div><MiniLabel>Student hint</MiniLabel><input value={t.hint} onChange={e => updateTurn(i, "hint", e.target.value)} style={inputStyle} placeholder="Hint shown to help the student respond" /></div>
              <div><MiniLabel>Ideal response</MiniLabel><textarea value={t.ideal} onChange={e => updateTurn(i, "ideal", e.target.value)} rows={2} placeholder="The ideal answer the student should give" style={{ ...inputStyle, resize: "none", lineHeight: 1.55 }} /></div>
              <div><MiniLabel>Grading criteria</MiniLabel><input value={t.criteria} onChange={e => updateTurn(i, "criteria", e.target.value)} style={inputStyle} placeholder="e.g. polite greeting used, verb tense correct, complete sentence" /></div>
            </div>
          </Card>
        ))}

        <button onClick={addTurn} style={{ width: "100%", padding: 11, borderRadius: 16, border: `2px dashed ${C.purpleMid}`, background: "transparent", cursor: "pointer", fontFamily: "'Nunito', sans-serif", fontSize: 13, fontWeight: 800, color: C.purple, marginBottom: 14 }}>＋ Add Turn</button>

        {window.RP_ACTIVITY_ID && (
          <div style={{ display: "flex", gap: 8, marginBottom: 10, alignItems: "center" }}>
            <Btn onClick={handleSave} color={C.purple} disabled={saving} style={{ flex: 1 }}>{saving ? "Guardando…" : "💾 Guardar"}</Btn>
            {saveMsg && (<span style={{ fontSize: 13, fontWeight: 700, color: saveMsg.startsWith("✓") ? C.greenText : "#DC2626" }}>{saveMsg}</span>)}
          </div>
        )}

        <Btn onClick={handleStart}>▶ Start Roleplay</Btn>
      </div>
    </div>
  );
}
...
function buildFeedback(transcript, target) {
  ...
  return {
    grammar: total, vocabulary: total, fluency: total, total,
...
function PlayerView({ scene, turns, onComplete, onBack, onListenFull, persistedResults = [], onResultsChange = null }) {
  ...
  useEffect(() => { if (!Array.isArray(persistedResults)) return; setResults(persistedResults); }, [persistedResults]);
  ...
  const speakWithVoice = useCallback(async (text, selectedVoiceId, opts = {}) => {
    ...
    try {
      ...
      if (e && e.code === "elevenlabs_request_failed" && window.speechSynthesis) {
        try { window.speechSynthesis.cancel(); const u = new SpeechSynthesisUtterance(text); u.lang = "en-US"; u.rate = 0.95; window.speechSynthesis.speak(u); } catch (err) {}
      }
      if (!silent) {
        const detail = e && e.message ? e.message : String(e);
        console.warn("Roleplay TTS error:", detail);
        if (!(e && e.code === "elevenlabs_request_failed")) alert("Could not play teacher audio: " + detail);
      }
    }
  ...
  function applyTurnResult(updatedTurnResult) {
    ...
    if (typeof onResultsChange === "function") onResultsChange(newResults);
    return newResults;
  }
  ...
  function advanceFromResults(turnIdx, sourceResults) {
    ...
    if (isAllCompleted) { onComplete(sourceResults); return; }
    let nextIdx = safeTurns.findIndex((_, idx) => idx > turnIdx && !completedTurns.has(idx));
    if (nextIdx === -1) nextIdx = safeTurns.findIndex((_, idx) => !completedTurns.has(idx));
    if (nextIdx === -1) nextIdx = Math.min(turnIdx + 1, safeTurns.length - 1);
  ...
  function evaluateBlock(turnIdx, transcript, options = {}) {
    ...
    const updatedTurnResult = { blockId: turnIdx, turnIdx, transcript: safeTranscript, feedback: fb };
    const newResults = applyTurnResult(updatedTurnResult);
    if (shouldResetRecorder) recorder.reset();
  ...
  function handleAdvanceCurrentTurn() {
    ...
    if (!transcript && existingResult) { advanceFromResults(currentTurn, results); return; }
    if (!transcript) { alert("Please record the student repetition first."); return; }
  ...
  function toggleRecordingForTurn(idx) {
    if (idx !== currentTurn) {
      stopAgentAudio(); recorder.reset(); setCurrentTurn(idx);
      setTimeout(() => { recorder.start(); }, 0);
      return;
    }
    if (recorder.isRecording) { pendingAutoEvalTurnRef.current = idx; recorder.stop(); return; }
    stopAgentAudio(); pendingAutoEvalTurnRef.current = null; recorder.start();
  }
        ...
        <div style={{ background: "#fff", border: "1px solid #EDE9FA", borderRadius: 24, overflow: "hidden" }}>
          ...
          <div style={{ padding: "14px 14px 110px", display: "flex", flexDirection: "column", gap: 12, background: "#F9F8FF" }}>
                ...
                <div key={idx} style={{ background: "#fff", border: isActive ? "2px solid #7F77DD" : "1.5px solid #EDE9FA", borderRadius: 18, padding: 12 }}>
                  <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", marginBottom: 8 }}>
                    ...
                    <div style={{ display: "flex", alignItems: "center", gap: 6 }}>
                      <span style={{ ...progressStyle, borderRadius: 999, padding: "4px 10px", fontSize: 11, fontWeight: 800 }}>{progressLabel}</span>
                      {!isActive && (
                        <button onClick={() => selectTurnForPractice(idx)} style={{ background: "#fff", color: "#7F77DD", border: "1.5px solid #EDE9FA", borderRadius: 999, padding: "4px 10px", fontSize: 11, fontWeight: 800, cursor: "pointer" }}>
                  ...
                  <div style={{ background: "#F5F3FF", border: "1px solid #EDE9FA", borderRadius: 14, padding: "10px 12px", marginBottom: 8 }}>
                    <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", gap: 8, marginBottom: 6, minHeight: 44 }}>
                      <div style={{ display: "flex", alignItems: "center", gap: 8 }}>
                        <div style={{ width: 44, height: 44, borderRadius: "50%", background: "#EDE9FA", color: "#5A51C0", display: "flex", alignItems: "center", justifyContent: "center", fontFamily: "'Nunito',sans-serif", fontSize: 18, fontWeight: 800, flexShrink: 0 }}>
                  ...
                  <div style={{ background: "#FFF7ED", border: "1px solid #FCDDBF", borderRadius: 14, padding: "10px 12px" }}>
                    <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", gap: 8, marginBottom: 6, minHeight: 44 }}>
                      <div style={{ display: "flex", alignItems: "center", gap: 8 }}>
                        <div style={{ width: 44, height: 44, borderRadius: "50%", background: "#FFF0E6", color: "#F97316", display: "flex", alignItems: "center", justifyContent: "center", fontSize: 20, flexShrink: 0 }}>🎓</div>
                    ...
                    {turnResult && turnResult.feedback && turnResult.feedback.corrected && (
                      <div style={{ marginTop: 6, background: "#FFF0E6", border: "1px solid #FCDDBF", borderRadius: 10, padding: "8px 10px" }}>
                        ...
                        <div style={{ fontSize: 12, fontWeight: 700, color: "#9A3412", lineHeight: 1.5 }}>{turnResult.feedback.corrected}</div>
                      </div>
                    )}
                        ...
                        return (
                          <>
                            <div style={{ display: "flex", gap: 8, flexWrap: "wrap" }}>
                              <button onClick={() => toggleRecordingForTurn(idx)} style={{ background: recorder.isRecording ? "#EF4444" : "#7F77DD", color: "#fff", border: "none", borderRadius: 12, padding: "10px 14px", fontSize: 13, fontWeight: 800, cursor: "pointer", minWidth: 94 }}>
                                {isActive && recorder.isRecording ? "Stop" : "Record"}
                              </button>
                              <button
                                onClick={() => { if (!isActive) { selectTurnForPractice(idx); return; } recorder.playRecording(); }}
                            ...
                            <div style={{ fontSize: 12, fontWeight: 800, color: isActive ? (currentTranscript ? (isCorrect ? "#166534" : "#B91C1C") : "#6A63B0") : "#6A63B0", minHeight: 18 }}>
                              {isActive
                                ? (currentTranscript ? `${isCorrect ? "Correct" : "Incorrect"}: ${currentTranscript}` : "Waiting for student response...")
                            ...
                            <div style={{ fontSize: 11, fontWeight: 700, color: "#6A63B0" }}>Use the fixed action bar below to continue.</div>
                          </>
                        );
                      })()}
                    </div>
            ...
            <div style={{ position: "fixed", left: "50%", transform: "translateX(-50%)", bottom: 12, width: "min(720px, calc(100vw - 24px))", zIndex: 999, background: "#FFFFFF", border: "1.5px solid #EDE9FA", borderRadius: 14, padding: "10px", display: "flex", gap: 8, justifyContent: "center", flexWrap: "wrap", boxShadow: "0 8px 20px rgba(127,119,221,.18)" }}>
              <button onClick={onListenFull} style={{ background: C.purple, color: "#fff", border: "none", borderRadius: 12, padding: "10px 14px", fontSize: 13, fontWeight: 800, cursor: "pointer", minWidth: 148 }}>
                Listen to roleplay
              </button>
              <button onClick={handleAdvanceCurrentTurn} disabled={!canAdvanceTurn} style={{ background: canAdvanceTurn ? "#F97316" : "#E5E1F8", color: canAdvanceTurn ? "#fff" : "#AAA2D8", border: "none", borderRadius: 12, padding: "10px 14px", fontSize: 13, fontWeight: 800, cursor: canAdvanceTurn ? "pointer" : "not-allowed", minWidth: 94 }}>
                Next
              </button>
            </div>
          </div>
...
function CompletionView({ scene, turns, results, onRetry }) {
  ...
  const progressPct = summary.totalBlocks > 0 ? Math.round((summary.completedBlocks / summary.totalBlocks) * 100) : 0;
  const title = scene.title || "Roleplay";
    ...
    <div style={{ background: "#F8F7FE", minHeight: "100%", padding: "clamp(14px,2.2vw,30px)", boxSizing: "border-box" }}>
      <div style={{ width: "min(940px,100%)", margin: "0 auto" }}>
        <div style={{ textAlign: "center", marginBottom: 16 }}>
          <span style={{ display: "inline-flex", alignItems: "center", justifyContent: "center", padding: "6px 14px", borderRadius: 999, background: "#FFF0E6", color: C.orange, fontSize: 11, fontWeight: 800, textTransform: "uppercase", letterSpacing: ".08em", marginBottom: 10 }}>Activity</span>
          <h1 style={{ margin: 0, fontFamily: "'Fredoka',sans-serif", fontSize: "clamp(30px,4.8vw,44px)", color: C.orange, lineHeight: 1.06, fontWeight: 600 }}>{title}</h1>
        ...
        <div style={{ width: "min(860px,100%)", margin: "0 auto", background: "#fff", border: "1px solid #EDE9FA", borderRadius: 24, boxShadow: "0 8px 40px rgba(127,119,221,.13)", padding: 18 }}>
          <div style={{ display: "grid", gridTemplateColumns: "auto 1fr auto", alignItems: "center", gap: 12, marginBottom: 14 }}>
            ...
            <div style={{ background: C.purple, color: "#fff", borderRadius: 999, padding: "5px 12px", fontSize: 12, fontWeight: 800, whiteSpace: "nowrap" }}>Q {summary.completedBlocks} of {summary.totalBlocks}</div>
          </div>

          <div style={{ display: "grid", gridTemplateColumns: "repeat(3,1fr)", gap: 10, marginTop: 12 }}>
            {[["Correct", summary.correctBlocks], ["Wrong", summary.wrongBlocks], ["Score", `${summary.scorePct}%`]].map(([lbl, val]) => (
              <div key={lbl} style={{ background: "#FAFAFE", border: "1px solid #EDE9FA", borderRadius: 14, padding: 12, textAlign: "center" }}>
                <div style={{ fontFamily: "'Fredoka',sans-serif", fontSize: 24, lineHeight: 1, fontWeight: 600, color: C.purple }}>{val}</div>
                <div style={{ marginTop: 3, fontSize: 10, fontWeight: 700, letterSpacing: ".08em", textTransform: "uppercase", color: "#bbb" }}>{lbl}</div>
              </div>
            ))}
          </div>
          ...
          <div style={{ marginTop: 14, background: "#fff", border: "1px solid #EDE9FA", borderRadius: 30, boxShadow: "0 12px 36px rgba(127,119,221,.13)", minHeight: "clamp(300px,42vh,430px)", display: "flex", flexDirection: "column", alignItems: "center", justifyContent: "center", textAlign: "center", padding: "clamp(28px,5vw,48px) 24px", gap: 10 }}>
            ...
            <button type="button" onClick={onRetry} style={{ marginTop: 6, border: "none", borderRadius: 999, color: "#fff", minWidth: 128, padding: "11px 20px", fontSize: 14, fontWeight: 700, fontFamily: "'Nunito',sans-serif", cursor: "pointer", background: C.purple }}>Restart</button>
          </div>
        </div>
      </div>
    </div>
...
function RoleplayActivity() {
  const allowEditor = !!window.RP_ALLOW_EDITOR;
  const initialView = window.RP_START_VIEW === "editor" && allowEditor ? "editor" : "player";
  const [view, setView] = useState(initialView);
  const [scene, setScene] = useState(canonicalizeScene(window.RP_SAVED_SCENE || DEFAULT_SCENE));
  const [turns, setTurns] = useState(window.RP_SAVED_TURNS || JSON.parse(JSON.stringify(DEFAULT_TURNS)));
  ...
  const playReplayLine = useCallback(async (text, voiceCandidates) => {
    ...
    for (const voiceId of candidateList) {
      try { const blob = await fetchElevenLabsAudioBlob(text, voiceId); await playAudioBlob(blob, null); return; } catch (err) {}
    }
    if (window.speechSynthesis) {
      await new Promise((resolve) => {
        try { const utt = new SpeechSynthesisUtterance(text); utt.lang = "en-US"; utt.rate = 1.0; utt.pitch = 1.08; utt.onend = () => resolve(); utt.onerror = () => resolve(); window.speechSynthesis.speak(utt); } catch (e) { resolve(); }
      });
      return;
    }
  ...
  const handleListenFull = useCallback(() => {
    ...
    const speakNext = async () => {
      if (listenCancelRef.current || i >= queue.length) { setIsListeningFull(false); return; }
      const item = queue[i++];
      try {
        ...
        console.warn("Roleplay full replay failed:", e && e.message ? e.message : e);
      }
      speakNext();
    };
    speakNext();
  ...
  const handleRetry = useCallback(() => { setResults([]); setView("player"); }, []);

  useEffect(() => {
    return () => { listenCancelRef.current = true; if (window.speechSynthesis) window.speechSynthesis.cancel(); };
  }, []);

  useEffect(() => {
    console.log("[roleplay] viewer bootstrap", {
      activityId: window.RP_ACTIVITY_ID || null,
      allowEditor, initialView,
      savedScene: window.RP_SAVED_SCENE,
      savedTurns: window.RP_SAVED_TURNS,
  ...
  return (
    <div>
      {allowEditor && view === "editor" && (
        <EditorView scene={scene} turns={turns} onSceneChange={setScene} onTurnsChange={setTurns} onStart={() => setView("player")} />
      )}
      {view === "player" && (
        <PlayerView
      ...
      {view === "completion" && (
        <CompletionView scene={scene} turns={turns} results={results} onRetry={handleRetry} />
      )}
    </div>
  );
}
