...
(function () {
  ...
  function normScene(s, blank) {
    s = s || {};
    if (blank) return {title:String(s.title || ''),scenario:String(s.scenario || s.description || ''),agentRole:String(s.agentRole || ''),studentRole:String(s.studentRole || ''),icon:String(s.icon || '🎭'),level:String(s.level || '')};
  ...
  function sameAsHintRequired(t) {
    const c = String((t && t.criteria) || '').toLowerCase();
    return c.includes('same answer as hint') || c.includes('same as hint') || c.includes('exactly as hint') || c.includes('must say hint') || c.includes('repeat the hint');
  }
  function grammarScore(ans) {
    const raw = String(ans || '').trim();
    const w = cleanWords(raw);
    let score = 35;
    if (w.length >= 3) score += 25;
    if (/^[A-Z]/.test(raw)) score += 10;
    if (/[.!?]$/.test(raw)) score += 10;
    if (/\b(i|you|he|she|we|they|it|there|this|that|the|a|an)\b/i.test(raw)) score += 10;
    if (/\b(am|is|are|was|were|do|does|did|have|has|had|can|could|will|would|like|want|need|go|eat|drink|order)\b/i.test(raw)) score += 10;
    if (/\b(\w+)\s+\1\b/i.test(raw)) score -= 15;
    if (w.length < 2) score -= 25;
  ...
  function completedPage() {
    ...
    const correct = scores.filter(s => (s.overallPct || 0) >= 70).length;
  ...
  function nextTurn() {
    ...
    scores[i] = scoreTurn(activeInput, turns[i] || {}, pronunciationScores[i]);
  ...
  function startMic(i) {
    ...
    rec.onresult = e => {
      ...
      activeInput = alt.transcript;
      pronunciationScores[i] = typeof alt.confidence === 'number' ? Math.round(alt.confidence * 100) : undefined;
  ...
  root.addEventListener('input', e => {
    ...
    if (el.dataset.answer) { activeInput = el.value; pronunciationScores[completed] = undefined; }
  });
  root.addEventListener('click', async e => {
    const btn = e.target.closest('[data-action]'); if (!btn) return;
    e.preventDefault();
    const a = btn.dataset.action;
    if (a === 'listen-agent') speakAgent(Number(btn.dataset.index));
    if (a === 'mic') startMic(Number(btn.dataset.index));
    if (a === 'show-answer') { shownAnswers[completed] = true; render(); }
    if (a === 'next') nextTurn();
    if (a === 'restart') { view = 'player'; completed = 0; answers = []; scores = []; shownAnswers = []; activeInput = ''; pronunciationScores = []; render(); }
    if (a === 'preview') { view = 'player'; completed = 0; activeInput = ''; answers = []; scores = []; shownAnswers = []; pronunciationScores = []; render(); }
