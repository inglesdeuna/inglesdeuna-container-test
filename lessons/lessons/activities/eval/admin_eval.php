      ...
      <div class="card">
        ...
        <table>
          ...
          <?php
            ...
            // rawurlencode preserva la URL interna como link clickeable en WhatsApp
            $waMsg     = rawurlencode('Hola! Te comparto tu examen: ' . ($currentExam['title'] ?? 'Examen') . "\nEntra aquí: " . $linkUrl);
            $emailSubj = rawurlencode('Examen: ' . ($currentExam['title'] ?? 'Examen'));
            $emailBody = rawurlencode("Hola,\n\nTe invitamos a realizar el examen \"" . ($currentExam['title'] ?? 'Examen') . "\".\n\nAccede desde el siguiente enlace:\n" . $linkUrl . "\n\nSaludos.");
          ?>
          <tr>
            ...
            <td style="white-space:nowrap;display:flex;flex-wrap:wrap;gap:4px;align-items:center;">
              ...
              <?php
                ...
                // Si el número colombiano no tiene código de país, agregar 57
                if ($waPhone && strlen($waPhone) === 10 && $waPhone[0] === '3') {
                    $waPhone = '57' . $waPhone;
                }
                // wa.me es el formato vigente (api.whatsapp.com/send está deprecado)
                $waHref = $waPhone
                    ? 'https://wa.me/' . $waPhone . '?text=' . $waMsg
                    : 'https://wa.me/?text=' . $waMsg;
              ?>
              <a class="btn btn-primary btn-sm" href="<?= h($waHref) ?>" target="_blank" rel="noopener noreferrer" title="Enviar por WhatsApp">📱 WA</a>
