...
function ev_identity_filter(string $studentName, string $studentDoc, string $studentEmail, string $studentPhone): array {
    if ($studentDoc !== '') {
        return ['sql' => "student_doc = ?", 'params' => [$studentDoc]];
    }
    if ($studentEmail !== '') {
        return ['sql' => "LOWER(COALESCE(student_email,'')) = LOWER(?)", 'params' => [$studentEmail]];
    }
    return [
        'sql' => "LOWER(student_name) = LOWER(?) AND COALESCE(student_phone,'') = ?",
        'params' => [$studentName, $studentPhone],
    ];
}
...
if ($isPreview) {
    ...
    if ($token !== '') {
        // Do not expire a valid link merely because its student capacity was reached.
        // Capacity is checked only when a NEW student starts. Returning students may
        // use all attempts configured in eval_exams.max_attempts.
        $stmt = $pdo->prepare("SELECT l.*,e.title AS exam_title,e.time_limit_min,e.max_attempts,e.instructions,e.cefr_level AS exam_cefr,e.status AS exam_status,e.modalities,e.unit_id AS exam_unit_id FROM eval_links l JOIN eval_exams e ON e.id=l.exam_id WHERE l.token=? AND (l.expires_at IS NULL OR l.expires_at>NOW()) LIMIT 1");
        $stmt->execute([$token]);
        $link = $stmt->fetch(PDO::FETCH_ASSOC) ?: null;
    }
}
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['start_exam'])) {
    ...
    if ($studentName === '') {
        ...
        $identity = ev_identity_filter($studentName, $studentDoc, $studentEmail, $studentPhone);

        // Attempts are per student and exam, independent from link capacity.
        $histSql = "SELECT id,status FROM eval_results WHERE exam_id=? AND " . $identity['sql'] . " AND status IN ('started','submitted')";
        $hist = $pdo->prepare($histSql);
        $hist->execute(array_merge([$examId], $identity['params']));
        $historyRows = $hist->fetchAll(PDO::FETCH_ASSOC) ?: [];
        $submittedAttempts = 0;
        foreach ($historyRows as $historyRow) {
            if (($historyRow['status'] ?? '') === 'submitted') $submittedAttempts++;
        }
        $attempt = $submittedAttempts + 1;

        // A link use represents one distinct student, not one attempt.
        $hasUsedThisLink = false;
        if (!$isPreview && !empty($link['id'])) {
            $usedSql = "SELECT 1 FROM eval_results WHERE link_id=? AND " . $identity['sql'] . " LIMIT 1";
            $usedStmt = $pdo->prepare($usedSql);
            $usedStmt->execute(array_merge([(int)$link['id']], $identity['params']));
            $hasUsedThisLink = (bool)$usedStmt->fetchColumn();
        }

        if ($attempt > (int)($link['max_attempts'] ?? 1)) {
            $errorMsg = 'Ya alcanzaste el número máximo de intentos.';
        } elseif (!$isPreview && !$hasUsedThisLink && (int)($link['uses_count'] ?? 0) >= (int)($link['max_uses'] ?? 1)) {
            $errorMsg = 'Este enlace alcanzó el número máximo de estudiantes.';
        } else {
            if ($isUnitExam) {
            ...
            if (!$isPreview && !empty($link['id']) && !$hasUsedThisLink) {
                $pdo->prepare('UPDATE eval_links SET uses_count=uses_count+1 WHERE id=?')->execute([$link['id']]);
            }
...
if ($step === 'result' && $resultRow) {
    ...
    $identity = ev_identity_filter(
        (string)($resultRow['student_name'] ?? ''),
        (string)($resultRow['student_doc'] ?? ''),
        (string)($resultRow['student_email'] ?? ''),
        (string)($resultRow['student_phone'] ?? '')
    );
    $attemptStmt = $pdo->prepare("SELECT COUNT(*) FROM eval_results WHERE exam_id=? AND " . $identity['sql'] . " AND status='submitted'");
    $attemptStmt->execute(array_merge([$examId], $identity['params']));
    $attemptsUsed = (int)$attemptStmt->fetchColumn();
    $canRetake = !$isPreview && $attemptsUsed < (int)($link['max_attempts'] ?? 1);
}
