...
<?php elseif ($step === 'quiz'): $qIndex=(int)($_GET['q'] ?? 0); $total=count($questions); $q=$questions[$qIndex] ?? null; ?>
  ...
  <div class="card"><div class="q-number"><?= h($q['type'] ?? 'question') ?> · <?= h($q['skill'] ?? '') ?></div><?php $evType=$q['type'] ?? 'multiple_choice'; ?><form method="POST" id="ev-quiz-form"><input type="hidden" name="eval_answer" value="1"><input type="hidden" name="q_index" value="<?= $qIndex ?>"><input type="hidden" name="q_total" value="<?= $total ?>"><?php if($evType==='pronunciation'): ?><div class="ev-pron-card"><?php if(!empty($q['image'])): ?><div class="ev-pron-image"><img src="<?= h($q['image']) ?>" alt="<?= h($q['correct'] ?? '') ?>"></div><?php endif; ?><div class="ev-pron-word"<?php if(empty($q['image'])): ?> style="font-size:clamp(26px,5vw,44px);margin:20px 0 8px;"<?php endif; ?>><?= h($q['correct'] ?? $q['question'] ?? '') ?></div><?php if(!empty($q['ph'])): ?><div class="ev-pron-ph"><?= h($q['ph']) ?></div><?php endif; ?><div class="ev-pron-cap" id="ev-pron-cap"></div></div><input type="hidden" name="answer" id="ev-pron-ans" value=""><div class="ev-pron-actions"><button type="button" class="ev-pron-btn ev-pron-listen" id="ev-pron-listen">🔊 Listen</button><button type="button" class="ev-pron-btn ev-pron-speak" id="ev-pron-speak">🎤 Speak</button><button type="submit" class="btn" name="skip" value="1">Skip</button></div><script>(function(){var expected=<?= json_encode((string)($q['correct']??'')) ?>;var audio=<?= json_encode((string)($q['audio']??'')) ?>;var cap=document.getElementById('ev-pron-cap'),ans=document.getElementById('ev-pron-ans');var submitted=false,curAudio=null,isSpeaking=false,isPaused=false,utter=null;function norm(t){return String(t||'').toLowerCase().trim().replace(/[.,!?;:'"]/g,'').replace(/\s+/g,' ');}function overlap(a,b){var wa=a.split(' ').filter(Boolean),wb=b.split(' ').filter(Boolean);if(!wa.length||!wb.length)return 0;var m=wa.filter(function(w){return wb.indexOf(w)!==-1}).length;return m/Math.max(wa.length,wb.length);}function isMatch(a,b){return a===b||overlap(a,b)>=0.8;}function doSubmit(ok){if(submitted)return;submitted=true;ans.value=ok?'1':'';setTimeout(function(){document.getElementById('ev-quiz-form').submit();},250);}function getVoice(){var vs=[];try{vs=speechSynthesis.getVoices()||[];}catch(e){}for(var i=0;i<vs.length;i++)if(String(vs[i].lang||'').toLowerCase()==='en-us')return vs[i];for(var j=0;j<vs.length;j++)if(String(vs[j].lang||'').toLowerCase().indexOf('en')===0)return vs[j];return null;}function listenFn(){if(audio){if(!curAudio){curAudio=new Audio(audio);curAudio.onended=function(){isSpeaking=false;isPaused=false;};}if(!curAudio.paused){curAudio.pause();isSpeaking=true;isPaused=true;}else{curAudio.play();isSpeaking=true;isPaused=false;}return;}if(!window.speechSynthesis)return;if(speechSynthesis.speaking&&!speechSynthesis.paused){speechSynthesis.pause();isSpeaking=true;isPaused=true;return;}if(speechSynthesis.paused||isPaused){speechSynthesis.resume();isSpeaking=true;isPaused=false;return;}speechSynthesis.cancel();utter=new SpeechSynthesisUtterance(expected);utter.lang='en-US';utter.rate=0.82;var v=getVoice();if(v)utter.voice=v;speechSynthesis.speak(utter);}function speakFn(){var C=window.SpeechRecognition||window.webkitSpeechRecognition;if(!C){doSubmit(false);return;}var r=new C();r.lang='en-US';r.interimResults=false;r.maxAlternatives=1;var heard=false;cap.textContent='';r.onresult=function(e){heard=true;var said=String(e.results&&e.results[0]&&e.results[0][0]?e.results[0][0].transcript:'');cap.textContent=said;doSubmit(isMatch(norm(said),norm(expected)));};r.onerror=function(){heard=true;doSubmit(false);};r.onend=function(){if(!heard)doSubmit(false);};r.start();}document.getElementById('ev-pron-listen').onclick=listenFn;document.getElementById('ev-pron-speak').onclick=speakFn;})();</script><?php elseif($evType==='dictation'): ?><div class="q-text"><?= h($q['question'] ?? 'Listen and type what you hear.') ?></div><?php if(!empty($q['image'])): ?><img src="<?= h($q['image']) ?>" style="max-width:100%;border-radius:14px;margin-bottom:12px;max-height:220px;object-fit:contain;"><br><?php else: ?><div style="font-size:52px;text-align:center;margin:12px 0;">🎧</div><?php endif; ?><button type="button" class="ev-listen-btn" id="ev-dict-listen">🔊 Listen</button><textarea class="text-answer" name="answer" placeholder="Type what you heard…" rows="3" style="resize:vertical;min-height:80px;"></textarea><script>(function(){var audio=<?= json_encode((string)($q['audio']??'')) ?>;var expected=<?= json_encode((string)($q['correct']??'')) ?>;var curAudio=null,isSpeaking=false,isPaused=false,utter=null;function getVoice(){var vs=[];try{vs=speechSynthesis.getVoices()||[];}catch(e){}for(var i=0;i<vs.length;i++)if(String(vs[i].lang||'').toLowerCase()==='en-us')return vs[i];for(var j=0;j<vs.length;j++)if(String(vs[j].lang||'').toLowerCase().indexOf('en')===0)return vs[j];return null;}function listen(){if(audio){if(!curAudio){curAudio=new Audio(audio);curAudio.onended=function(){isSpeaking=false;isPaused=false;};}if(!curAudio.paused){curAudio.pause();isPaused=true;}else{curAudio.play();isPaused=false;}return;}if(!window.speechSynthesis)return;if(speechSynthesis.speaking&&!speechSynthesis.paused){speechSynthesis.pause();isSpeaking=true;isPaused=true;return;}if(speechSynthesis.paused||isPaused){speechSynthesis.resume();isSpeaking=true;isPaused=false;return;}speechSynthesis.cancel();utter=new SpeechSynthesisUtterance(expected);utter.lang='en-US';utter.rate=0.82;var v=getVoice();if(v)utter.voice=v;speechSynthesis.speak(utter);}document.getElementById('ev-dict-listen').onclick=listen;setTimeout(listen,200);})();</script><?php elseif($evType==='drag_drop'): ?><?php $evDdWords=$q['options']??[];$evDdShuffled=$evDdWords;shuffle($evDdShuffled);$evDdParts=preg_split('/(___)/',$q['instruction']??'',-1,PREG_SPLIT_DELIM_CAPTURE);$evDdSlotCount=count($q['correct_words']??[]);?><div class="q-text"><?= h($q['question'] ?? 'Drag the words into the correct blanks.') ?></div><?php if(!empty($q['listen_enabled'])||!empty($q['audio'])): ?><button type="button" class="ev-listen-btn" id="ev-dd-listen">🔊 Listen</button><?php endif; ?><?php if(!empty($q['image'])): ?><img src="<?= h($q['image']) ?>" style="max-width:100%;border-radius:14px;margin-bottom:12px;max-height:220px;object-fit:contain;"><?php endif; ?><div class="ev-dd-instr" id="ev-dd-instr"><?php $evDdSlotsDone=0;foreach($evDdParts as$evDdPart){if($evDdPart==='___'&&$evDdSlotsDone<$evDdSlotCount){?><div class="ev-dd-slot" data-slot-index="<?= $evDdSlotsDone ?>"><span class="ev-dd-ph">Drop here</span></div><?php $evDdSlotsDone++;}elseif($evDdPart!==''){?><span class="ev-dd-text"><?= h($evDdPart) ?></span><?php }}?></div><div id="ev-dd-bank" class="ev-word-bank"><?php foreach($evDdShuffled as$evDdWord):?><span class="ev-chip ev-bank-chip" draggable="true" data-word="<?= h($evDdWord) ?>"><?= h($evDdWord) ?></span><?php endforeach;?></div><div id="ev-dd-inputs"></div><script>(function(){var bank=document.getElementById('ev-dd-bank');var inputs=document.getElementById('ev-dd-inputs');var slots=Array.from(document.querySelectorAll('.ev-dd-slot'));var listenBtn=document.getElementById('ev-dd-listen');var dragged=null;var listenText=<?= json_encode((string)($q['listen_text']??$q['correct']??'')) ?>;var listenAudio=<?= json_encode((string)($q['audio']??'')) ?>;var listenPlayer=null;function clearSlot(slot){var chip=slot.querySelector('.ev-built-chip');if(chip){bank.appendChild(createBankChip(chip.dataset.word||chip.textContent.trim()));chip.remove();}slot.classList.remove('ev-dd-filled');slot.innerHTML='<span class="ev-dd-ph">Drop here</span>';}function fillSlot(slot,word){clearSlot(slot);var chip=createBuiltChip(word);slot.innerHTML='';slot.appendChild(chip);slot.classList.add('ev-dd-filled');syncInputs();}function createBankChip(word){var c=document.createElement('span');c.className='ev-chip ev-bank-chip';c.draggable=true;c.dataset.word=word;c.textContent=word;c.addEventListener('dragstart',function(e){dragged=c;c.dataset.src='bank';if(e.dataTransfer)e.dataTransfer.effectAllowed='move';});c.addEventListener('click',function(){var empty=slots.find(function(s){return !s.classList.contains('ev-dd-filled');});if(!empty)return;fillSlot(empty,word);c.remove();});return c;}function createBuiltChip(word){var c=document.createElement('span');c.className='ev-chip ev-built-chip';c.draggable=true;c.dataset.word=word;c.textContent=word;c.addEventListener('dragstart',function(e){dragged=c;c.dataset.src='slot';if(e.dataTransfer)e.dataTransfer.effectAllowed='move';});c.addEventListener('click',function(){var slot=c.parentElement;if(slot)clearSlot(slot);syncInputs();});return c;}function syncInputs(){inputs.innerHTML='';slots.forEach(function(slot,index){var chip=slot.querySelector('.ev-built-chip');if(!chip)return;var inp=document.createElement('input');inp.type='hidden';inp.name='answer['+index+']';inp.value=chip.dataset.word||chip.textContent.trim();inputs.appendChild(inp);});}slots.forEach(function(slot){slot.addEventListener('dragover',function(e){e.preventDefault();slot.classList.add('ev-drag-over');});slot.addEventListener('dragleave',function(){slot.classList.remove('ev-drag-over');});slot.addEventListener('drop',function(e){e.preventDefault();slot.classList.remove('ev-drag-over');if(!dragged)return;var word=dragged.dataset.word||dragged.textContent.trim();if(dragged.dataset.src==='slot'&&dragged.parentElement===slot){dragged=null;return;}if(dragged.dataset.src==='slot'&&dragged.parentElement)clearSlot(dragged.parentElement);fillSlot(slot,word);if(dragged.dataset.src==='bank')dragged.remove();dragged=null;});slot.addEventListener('click',function(){if(!slot.classList.contains('ev-dd-filled'))return;clearSlot(slot);syncInputs();});});bank.addEventListener('dragover',function(e){e.preventDefault();});bank.addEventListener('drop',function(e){e.preventDefault();if(!dragged||dragged.dataset.src!=='slot')return;if(dragged.parentElement)clearSlot(dragged.parentElement);dragged=null;syncInputs();});Array.from(bank.querySelectorAll('.ev-bank-chip')).forEach(function(chip){chip.addEventListener('dragstart',function(e){dragged=chip;chip.dataset.src='bank';if(e.dataTransfer)e.dataTransfer.effectAllowed='move';});chip.addEventListener('click',function(){var empty=slots.find(function(s){return !s.classList.contains('ev-dd-filled');});if(!empty)return;fillSlot(empty,chip.dataset.word||chip.textContent.trim());chip.remove();});});if(listenBtn){listenBtn.addEventListener('click',function(){if(listenPlayer){listenPlayer.pause();listenPlayer.currentTime=0;listenPlayer=null;}try{if(window.speechSynthesis)window.speechSynthesis.cancel();}catch(e){}if(listenAudio){listenPlayer=new Audio(listenAudio);listenPlayer.play().catch(function(){});return;}if(!listenText||!window.speechSynthesis)return;var utter=new SpeechSynthesisUtterance(listenText);utter.lang='en-US';utter.rate=0.85;window.speechSynthesis.speak(utter);});setTimeout(function(){listenBtn.click();},200);}})();</script><?php elseif($evType==='writing_practice'): ?><?php if(!empty($q['instruction'])): ?><div class="ev-wp-badge"><?= h($q['instruction']) ?></div><?php endif; ?><div class="q-text"><?= h($q['question'] ?? $q['prompt_text'] ?? '') ?></div><?php if(!empty($q['audio'])): ?><button type="button" class="ev-listen-btn" id="ev-wp-listen">🔊 Listen</button><script>(function(){var a=<?= json_encode((string)$q['audio']) ?>;var p=null;document.getElementById('ev-wp-listen').onclick=function(){if(p){p.pause();p.currentTime=0;p=null;return;}p=new Audio(a);p.play().catch(function(){});};})();</script><?php endif; ?><textarea class="text-answer" name="answer" placeholder="Write your answer…" rows="4" style="resize:vertical;"></textarea><?php elseif(!empty($q['options'])): ?><div class="q-text"><?= h($q['question'] ?? $q['text'] ?? '') ?></div><?php if(!empty($q['audio'])): ?><audio controls src="<?= h($q['audio']) ?>" style="width:100%;margin-bottom:12px"></audio><?php endif; ?><?php if(!empty($q['image'])): ?><img src="<?= h($q['image']) ?>" style="max-width:100%;border-radius:14px;margin-bottom:12px;max-height:220px;object-fit:contain;"><?php endif; ?><?php if(!empty($q['passage'])): ?><div style="border:1.5px solid var(--line);border-radius:14px;padding:14px 16px;margin-bottom:16px;line-height:1.7;font-size:15px;background:#fbfaff;"><?= nl2br(h($q['passage'])) ?></div><?php endif; ?><div class="options-list"><?php foreach ($q['options'] as $i=>$opt): ?><label class="option-label"><input type="radio" name="answer" value="<?= $i ?>"><span class="opt-letter"><?= chr(65+$i) ?></span><span><?= h((string)$opt) ?></span></label><?php endforeach; ?></div><?php else: ?><div class="q-text"><?= h($q['question'] ?? $q['text'] ?? '') ?></div><?php if(!empty($q['audio'])): ?><audio controls src="<?= h($q['audio']) ?>" style="width:100%;margin-bottom:12px"></audio><?php endif; ?><?php if(!empty($q['image'])): ?><img src="<?= h($q['image']) ?>" style="max-width:100%;border-radius:14px;margin-bottom:12px;max-height:220px;object-fit:contain;"><?php endif; ?><textarea class="text-answer" name="answer" placeholder="Escribe tu respuesta"></textarea><?php endif; ?><div style="display:flex;gap:10px;margin-top:22px;"><button class="btn btn-purple" type="submit">Continuar</button><button class="btn" type="submit" name="skip" value="1">Saltar</button></div></form></div>
  <?php endif; ?>
