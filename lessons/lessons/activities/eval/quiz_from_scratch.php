...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if ($action === 'assign_unit') {
        $examId = (int)rq('exam_id', '0');
        $unitId = rq('unit_id');
        if ($examId <= 0) $error = 'Examen no valido.';
        elseif ($unitId === '') $error = 'Selecciona una unidad.';
        else {
            ...
            go('quiz_from_scratch.php?mode=select&unit=' . urlencode($unitId) . '&exam_id=' . $examId . '&msg=unit_assigned');
        }
    }
    ...
    if ($action === 'create_exam') {
        ...
        if ($unitId === '') $error = 'Selecciona una unidad antes de crear bloques.';
        elseif ($title === '') $error = 'Escribe el nombre del examen.';
        elseif (!$selected) $error = 'Selecciona al menos un bloque evaluable.';
            ...
            if ($examId <= 0) {
                ...
                $stmt = $pdo->prepare("INSERT INTO eval_exams
                    ...
                    VALUES (?, NULL, ?, 50, 1, 'draft', ?, ?, ?) RETURNING id");
                $stmt->execute([$title, $unitId, json_encode(['online','printed']), $createdBy, 'Examen creado desde cero con bloques evaluables.']);
                $examId = (int)$stmt->fetchColumn();
            } else {
                $pdo->prepare('UPDATE eval_exams SET title=?, unit_id=? WHERE id=?')->execute([$title, $unitId, $examId]);
            }
...
$backHub = $unitId !== '' ? '/lessons/lessons/activities/hub/index.php?unit=' . urlencode($unitId) : 'admin_eval.php?tab=editor' . ($examId ? '&exam_id=' . $examId : '');
$showAssignUnit = ($examId > 0 && $unitId === '');
$showSelectBlocks = (!$showAssignUnit && ($mode !== 'edit' || !$exam || !$activities));
?>
<!DOCTYPE html>
<html lang="es">
<head>
...
<?php if ($msg === 'created'): ?><div class="msg">Examen creado. Edita cada bloque y luego revisa online o impreso.</div><?php endif; ?><?php if ($msg === 'saved'): ?><div class="msg">Examen guardado. Ahora selecciona la unidad y los bloques evaluables.</div><?php endif; ?><?php if ($error): ?><div class="msg err"><?= h($error) ?></div><?php endif; ?>
<section class="hero"><h1><?= $exam ? h($exam['title']) : 'Crear examen desde cero' ?></h1><p>Este flujo crea exámenes con actividades evaluables y abre sus editores. No usa el modal manual de preguntas.</p></section>
