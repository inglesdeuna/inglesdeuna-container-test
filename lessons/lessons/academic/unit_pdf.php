...
function ws_dictation(array $d, int $n, bool $k): string {
    ...
    foreach ($items as $i => $item) {
        ...
        $numLines = $img !== '' ? 3 : pdf_answer_line_count($en);
...
function ws_pronunciation(array $d, int $n, bool $k): string {
    $items = is_array($d['items'] ?? null) ? $d['items'] : [];
    $out = ws_head($n, 'Pronunciation', trim((string)($d['title'] ?? '')), 'Read each word, pronunciation and example.', $k, 'card-open pr-print pr-card-print');
    if (empty($items)) return $out.'<div class="notes-box"></div>'.ws_foot();
    $out .= '<div class="pr-print-grid">';
    foreach ($items as $i => $item) {
        if (!is_array($item)) continue;
        $word = pdf_pick($item, ['en','word','text']);
        $ipa = pdf_pick($item, ['ipa']);
        $ph = pdf_pick($item, ['ph']);
        $meaning = pdf_pick($item, ['meaning','es','spanish']);
        $example = pdf_pick($item, ['example','sentence']);
        $img = pdf_pick($item, ['img','image']);
        if ($word==='' && $img==='') continue;
        $out .= '<div class="pr-print-card">';
        $out .= '<div class="pr-print-card-num">'.($i+1).'</div>';
        if ($img !== '') $out .= '<div class="pr-print-img"><img src="'.h($img).'" alt="" loading="eager"></div>';
        $out .= '<div class="pr-print-word">'.h($word).'</div>';
...
$studentFieldsPatch = <<<'PHP'
<!-- Student fields -->
<div class="student-grid">
  <div class="sf"><span class="sf-lbl">Name:</span><div class="sf-line"></div></div>
  <div class="sf"><span class="sf-lbl">Date:</span><div class="sf-line"></div></div>
</div>

<!-- General instructions -->
PHP;
$source = preg_replace('/<!-- Student fields -->[\s\S]*?<!-- General instructions -->/', $studentFieldsPatch, $source, 1);
...
$worksheetCss = <<<'CSS'
.student-grid{grid-template-columns:1fr 1fr!important;gap:12px!important}.sf{padding:11px 12px!important;display:flex!important;align-items:center!important;gap:8px!important}.sf-lbl{font-size:9px!important;letter-spacing:.08em!important;white-space:nowrap!important;margin:0!important}.sf-line{flex:1!important;height:12px!important}
